...
- **Recientes (rama)**:
  ...
  - c.271 (`9904e1c`): **P1 parser recurrencia ordinal con cadencia PRECEDENTE** - "gym cada mes el primer lunes" producía MONTHLY `days=''` (motor anclaba al día del mes → 2ª cita derivaba a otro weekday) Y dejaba "el primer" como residuo del título. RESUELTO: nuevo `precedingCadenceOrdinalPattern` captura ordinal+weekday cuando la cadencia va ANTES ("cada mes/mensual/todos los meses el primer lunes"); `OrdinalMonthlyCapture` unifica la forma directa y la precedente para anclar el motor al N-ésimo/último weekday del mes. Adicional: `recurrenceAdjectiveLeakPattern` ahora cubre la forma intervalo-1 "cada \<unidad>" ("reunión cada semana los lunes" → "reunión", paridad con "semanal los lunes"). +7 tests (4 ordinal precedente + 3 leak intervalo-1).
  - c.270 (`c787ac7`): **P0 what-now/TodayScreen build break** - `when` no exhaustivo en `TodayScreen.whatNowReasonLabel` rompía el build (falta rama `MISSIED_START`). Fix UI (fuera del harness JVM). NO VERIFICADO JVM-Android.
  - c.269 (`bf08c3d`): **P2 búsqueda/recuperación simetría captura↔búsqueda** - búsqueda universal NO recuperaba por FIN DE SEMANA ("finde"/"fin de semana") pese a que el parser de captura SÍ crea tareas de finde. Nuevo `DateScope.WEEKEND` + `isWeekendQuery` + `resolveWeekendTarget`. +9 tests.
  ...
  - c.267 (`ee460d6`): **feat(guardian) nudge diario nombra capturas arrinconadas** - cuando no hay atrasadas ni huecos perdidos pero hay una captura en bandeja (>=7 días, sin fecha, sin impulso real del día), el nudge la nombra ("«X» lleva 2 semanas en tu bandeja sin fecha. Hazla hoy, agéndala o quítala.") en vez del mensaje genérico. Adicional: fix de consistencia de zona - `staleInboxAction` usaba `ZoneId.systemDefault()`; ahora propaga `zoneId` (snapshot -> suggestedAction -> staleInboxAction), correcto en cualquier zona (verificado con systemDefault=UTC+14). +5 tests.
  - c.266 (`58d8934`): **P1 fuente-única recurrencia+notificación perdía el desglose** - completar una recurrente CON subtareas desde el RECORDATORIO no clonaba el checklist (sólo la app lo hacía, c.223/c.236). RESUELTO: orquestador `RecurrenceSpawn.spawnNextOccurrence` unifica los 4 caminos spawn (2 ViewModel + 2 ReminderActionReceiver). Composición de reglas puras ya verificadas; orquestador Android = NO VERIFICADO JVM.
...
- **OPEN pendientes** (mayor impacto primero):
  - P2 "bisemanal" ambiguo (parser) — probe anterior identificó `bisemanal`→freq=NONE (debería WEEKLY/2) y `bisemanal los lunes`→interval=1 (debería 2). Cadencia NO mensual (no reusa el fix c.273), requiere rama WEEKLY propia.
  - P3 residuos de título restantes ("a las 3.5" decimal-hour). (El residuo "el primer" suelto de cadencia precedente quedó RESUELTO c.271; el residuo "cada \<unidad>" intervalo-1 quedó RESUELTO c.271; el "de \<mes>" de límites mensuales quedó RESUELTO c.265; el ordinal plurimensual precedente quedó RESUELTO c.273.)
...
- **Próxima prioridad**: descubrimiento continuo - (i) P2 "bisemanal"/"quincenal" como cadencia WEEKLY propia (probe ya identificó los fallos: freq=NONE e interval=1 — clase DISTINTA a c.273, no reusa el patrón mensual); (ii) seguir auditando el parser por paridad léxica residual (c.273 cerró la rendija plurimensual de c.271; c.269 cerró la simetría captura↔búsqueda finde); (iii) seguir auditando motores no-parser por rendijas simétricas (c.268 cerró la de `DayPlanner`; c.270 cerró WhatNowReason build-break + auditó los 39 sitios `when`-on-enum sin `else` en UI no-domain -> todos exhaustivos; GuardianEngine/GuardianCoach ya alineados con stale-inbox c.267/c.259); (iv) auditar OTROS orquestadores Android con copias duplicadas que puedan divergir (patrón simétrico al de c.266); (v) areas no-parser (onboarding, navegacion, accesibilidad, rendimiento, workers/backup con DAOs reales). Re-fetch antes de implementar.
