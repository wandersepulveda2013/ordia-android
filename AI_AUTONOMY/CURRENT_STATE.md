...
- **Próxima prioridad**: descubrimiento continuo NO-parser (siguiendo guía supervisor de apartarse del parser) - (i) seguir auditando motores no-parser por rendijas simétricas (c.277 cerró la de `DayPlanner` en-curso vs `timeRank` de What Now/widget; c.268 cerró la de `DayPlanner` bandeja; c.270 cerró WhatNowReason build-break + auditó los 39 sitios `when`-on-enum sin `else` en UI no-domain -> todos exhaustivos; GuardianEngine/GuardianCoach ya alineados con stale-inbox c.267/c.259); AUDITORÍA c.277 (post-fix) de los orquestadores que ordenan tareas y los motores de recordatorios/recurrencia: `TaskRules.timeRank`/`nextBestTask` (en-curso = rango más alto, OK), `WhatNowEngine` (alineado con timeRank, OK), `GuardianEngine` (`smallestOverdueAction`/`missedStartAction` filtran `isInProgressNow` explícitamente, OK), `SummaryEngine.mostDeferrableTask` (filtra `isInProgressNow`, OK), `ReminderRules` (`snooze`/`resolveReminderAt`/`defaultReminderAt` past-safe y offset-preserving, OK), `RecurrenceEngine.nextOccurrence` (traslada reminderOffset/startOffset con fallback past-safe, OK) -> TODOS limpios, `DayPlanner` era la única rendija (cerrada c.277); `PlannerCalendar.taskComparator` es cronológico (startAt→due→priority) y es CORRECTO para una vista de calendario (forzar en-curso-primero rompería la cronología: NO es bug); concluir: NO hay bug simétrico residual en ordenamiento/recordatorios/recurrencia del dominio puro - no fabricar cambio aquí; (ii) auditar OTROS orquestadores Android con copias duplicadas que puedan divergir (patrón simétrico al de c.266); (iii) areas no-parser (onboarding, navegacion, accesibilidad, rendimiento, workers/backup con DAOs reales); (iv) parser residual sólo si aparece una rendija P0/P1 clara (la paridad léxica está muy cerrada: c.93/c.271/c.273/c.275/c.276). Re-fetch antes de implementar.
