...
## Estado
- **Fecha (UTC)**: 2026-08-15. Rama `openhands/autonomous-ordia`. Este run es **c.289** (P1 evitar olvidos / consistencia entre superficies / IA honesta, área "tarjeta de insight / evitar olvidos / recuperación de información importante"): la tarjeta de insight permanente (`GuardianCoach.insight`) —la superficie MÁS visible de recuperación, el usuario la ve siempre en la pantalla principal— recuperaba los TRES olvidos de tareas (atrasada / hueco incumplido / captura arrinconada) PERO callaba el CUARTO — el compromiso vencido extraído de una conversación (`CommitmentRules.isOverduePending` de c.286). c.286 lo cerró en el asistente (superficie a demanda "¿qué olvidé?"); c.288 en el nudge del guardián (superficie PROACTIVA); este run la cierra en la tarjeta PERMANENTE. RESUELTO: `GuardianCoach.insight` añade `commitments: List<CommitmentEntity> = emptyList()` (backward-compatible); nueva rama `overdueCommitments.isNotEmpty()` nombra el compromiso más atrasado + "Revísalo en Conversaciones" (paridad con c.286/c.288) DESPUÉS de los olvidos de tarea (lo accionable en una tarea ya capturada manda) y ANTES de la captura arrinconada (una promesa vencida es señal más fuerte que una captura sin fecha); más cola informativa `withCommitmentTail` cuando la tarjeta YA nombra una tarea olvidada (no calla el compromiso, no doble señalización). Tono honesto por edad de calendario (mismo umbral/helper `overdueDays` que las vencidas). **Cableado UI**: `OrdiaViewModel.guardianInsight` pasa `commitments = commitments` (`OrdiaUiState.commitments` ya existe de c.288). Sin nueva pantalla/botón, sin IA fingida (predicado `CommitmentRules` determinista de c.286). Base inicial `b80b5f1` (c.288). Entorno JVM (sin Android SDK): kotlinc 2.1.20, jars en `/tmp/libs`, OpenJDK 21.
- **Tests**: `bash tools/run_domain_tests.sh` -> **1855 PASS** (1850 c.288 + 5 c.289), 0 failures, 41 clases; `bash tools/run_domain_checks.sh` -> smoke 25 OK (lista actualizada: añadí `CommitmentRules.kt` —nueva dependencia de `GuardianCoach`— al subconjunto del smoke, sin él el smoke no compilaba pese a que la suite completa sí). **NO VERIFICADO** gradle/lint/assemble/Android/UI/Room con DAOs reales (sin Android SDK).
- **Recientes (rama)**:
  - c.289 (este run): **P1 evitar olvidos / consistencia entre superficies — la tarjeta de insight permanente callaba la cuarta clase de olvido: compromisos vencidos de conversaciones** - `GuardianCoach.insight` recuperaba los tres olvidos de tareas (atrasada / hueco incumplido / captura arrinconada) PERO callaba el compromiso vencido extraído de una conversación (`CommitmentRules.isOverduePending`: `dueAt` pasado, `PENDING`, sin convertir en tarea). c.286 cerró la rendija en el asistente (a demanda "¿qué olvidé?"); c.288 en el nudge del guardián (proactivo); este run la cierra en la tarjeta PERMANENTE — la superficie MÁS visible (el usuario la ve siempre en la pantalla principal). Así un día sin vencidas ni huecos pasados pero con una promesa vencida mostraba "SIGUIENTE PASO" o "TODO EN CALMA" — mintiendo por omisión; y si SÍ había tarea vencida, nombraba la tarea y callaba el compromiso. RESUELTO: `insight` recibe `commitments: List<CommitmentEntity> = emptyList()` (backward-compatible); nueva rama `overdueCommitments.isNotEmpty()` DESPUÉS de overdue/missed (lo accionable en una tarea ya capturada manda) y ANTES de la captura arrinconada (una promesa con plazo vencido es señal más fuerte que una captura sin fecha) + cola `withCommitmentTail` cuando ya nombró una tarea olvidada (no la calla, no doble señalización: la acción primaria sigue siendo la tarea). El compromiso no es tarea hasta convertirse → acción es guiar "Revísalo en Conversaciones" (paridad con c.286/c.288), no un taskId. Tono honesto por edad de calendario (`overdueDays` + `FORGOTTEN_DAYS_THRESHOLD`, mismo helper que las vencidas). **Cableado UI**: `OrdiaViewModel.guardianInsight` pasa `commitments = commitments` (`OrdiaUiState.commitments` ya existe de c.288). Sin nueva pantalla/botón, sin IA fingida (predicado `CommitmentRules` determinista de c.286). +5 tests TDD. Área ADYACENTE a c.286/c.288 (mismo "4º olvido", superficie distinta: tarjeta permanente vs asistente a demanda vs nudge proactivo).
