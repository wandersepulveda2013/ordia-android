...
## Estado
- **Fecha (UTC)**: 2026-08-15. Rama `openhands/autonomous-ordia`. Este run es **c.290** (P0 privacidad / datos (sagrados), área "privacidad/seguridad" — continuación directa de c.287): **fuga de claves privadas cripto por el gate de notificaciones.** `ConversationPrivacyPolicy` (el filtro que decide si una notificación de SMS/mensajería se **persiste** en la BD de conversaciones) NO bloqueaba claves privadas: 64-hex (con/sin prefijo `0x`, clave Ethereum/cripto) ni bloques PEM `-----BEGIN ... PRIVATE KEY-----`. El gate de contexto/IME (`ContextPrivacyFilter`, sólo lectura) SÍ los bloqueaba — la MISMA asimetría que c.287 cerró para saldo/IBAN/seed phrase. Así un SMS "guarda tu clave: 0x4c08...4d" pasaba el filtro de paquete (no bancario) y se **persistía en texto plano**. Confirmado con probe JVM reproducible (3/3 leaks: `blocked? false` para eth-privkey/bare-hex/PEM en `ConversationPrivacyPolicy`; `true` en `ContextPrivacyFilter`). RESUELTO: `ConversationPrivacyPolicy.sensitivePatterns` añade los 2 patrones **idénticos** a `ContextPrivacyFilter` (líneas 33-34) — PEM block + `\b(?:0x)?[0-9a-f]{64}\b`. Ambos gates ahora bloquean las mismas categorías (paridad completa c.287→c.290). Sin nueva pantalla/botón. Base inicial `8dda389` (c.289). Entorno JVM (sin Android SDK): kotlinc 2.1.20, jars en `/tmp/libs`, OpenJDK 21.
- **Tests**: `bash tools/run_domain_tests.sh` -> **1856 PASS** (1855 c.289 + 1 c.290), 0 failures, 41 clases; `bash tools/run_domain_checks.sh` -> smoke 25 OK. **NO VERIFICADO** gradle/lint/assemble/Android/UI/Room con DAOs reales (sin Android SDK).
- **Recientes (rama)**:
  - c.290 (este run): **P0 privacidad / datos (sagrados) — el gate de notificaciones persistía claves privadas cripto en texto plano** - `ConversationPrivacyPolicy` (filtro que decide si una notificación de SMS/mensajería se **persiste** en la BD de conversaciones) NO bloqueaba claves privadas: 64-hex (con/sin prefijo `0x`, clave Ethereum/cripto) ni bloques PEM `-----BEGIN ... PRIVATE KEY-----`. El gate de contexto/IME (`ContextPrivacyFilter`, sólo lectura) SÍ los bloqueaba — MISMA asimetría que c.287 cerró para saldo/IBAN/seed phrase. Así un SMS "guarda tu clave: 0x4c08...4d" pasaba el filtro de paquete (no bancario) y se persistía en texto plano → pérdida directa de fondos. Confirmado con probe JVM reproducible (3/3 leaks). RESUELTO: `ConversationPrivacyPolicy.sensitivePatterns` añade los 2 patrones **idénticos** a `ContextPrivacyFilter` (líneas 33-34): PEM block + `\b(?:0x)?[0-9a-f]{64}\b`. Ambos gates ahora bloquean las mismas categorías (paridad completa c.287→c.290). +1 test TDD (RED→GREEN). Área ORTOGONAL a c.289 (privacidad vs evitar olvidos), archivos DISJUNTOS.
