...
## Estado
- **Fecha (UTC)**: 2026-08-15. Rama `openhands/autonomous-ordia`. Este run es **c.283** (NO-parser, área "orquestadores de tareas"): **CIERRA la consolidación `isBeingWorkedOn` que c.280 creyó completa** — `GuardianCoach` (la tarjeta de insight, la superficie MÁS visible de recuperación) seguía usando el predicado parcial `isInProgressNow` en su filtro `recoverable`, así que una atrasada marcada `IN_PROGRESS` a mano SIN ventana activa era nombrada como "comienza por esta" aunque el usuario ya la hiciera. Exactamente la divergencia que c.280 cerró en `GuardianEngine.smallestOverdueAction`. Base inicial `2017135` (c.280); durante el run el remoto avanzó DOS veces por runs paralelos que tomaron c.281 ("pasado mañana") y c.282 ("entre lunes y viernes", parser), ambos en archivos DISJUNTOS de mi `GuardianCoach`. Integración NO destructiva vía fetch→ff-only→restaurar código→rebuild `.md` (sin force push/reset --hard): `.kt` disjuntos sin conflictos. Cedido c.281/c.282 a los runs paralelos; este run renumerado a **c.283**. Entorno JVM (sin Android SDK): kotlinc 2.1.20, jars en `/tmp/libs`, OpenJDK 21.
- **Tests**: `bash tools/run_domain_tests.sh` -> **1817 PASS** (1816 c.282 + 1 c.283), 0 failures, 40 clases; `bash tools/run_domain_checks.sh` -> smoke 25 OK. **NO VERIFICADO** gradle/lint/assemble/Android/UI/Room con DAOs reales (sin Android SDK).
- **Recientes (rama)**:
  - c.283 (este run): **P2 consistencia entre superficies / inteligencia honesta / MENOS ES MÁS — orquestadores "en curso" (cierre de c.280)** - c.280 declaró completa la consolidación del predicado "sacro en curso" `isBeingWorkedOn` en "5 orquestadores", PERO omitió `GuardianCoach` (la tarjeta de insight del asistente, su propia KDoc la llama "la superficie MÁS visible de recuperación"). Allí el filtro `recoverable = overdue.filter { !isInProgressNow(it) }` usaba el predicado PARCIAL `isInProgressNow` (sólo ventana de tiempo), NO el canónico `isBeingWorkedOn` (= `status==IN_PROGRESS || isInProgressNow`). Resultado: una tarea atrasada marcada `IN_PROGRESS` a mano SIN ventana activa (p. ej. el usuario la arrancó, su hueco `startAt` ya pasó) caía en `recoverable` y `nextBestTask` la encabezaba por `timeRank` (IN_PROGRESS = rango máx) → el coach nudgiaba **"RECUPERA EL CONTROL / comienza por esta"** sobre la tarea que el usuario YA hace. Misma incoherencia que c.280 cerró en `GuardianEngine.smallestOverdueAction`; la KDoc del coach incluso reclamaba paridad con esa función, paridad rota tras c.280. RESUELTO: `!isInProgressNow` → `!isBeingWorkedOn` (1 línea + KDoc actualizada). Sin nueva pantalla/botón, sin IA fingida. +1 test TDD (`manuallyInProgressOverdueWithoutActiveWindowIsNotNamedAsRecoverable`, RED→GREEN: expected:<20> but was:<10> pre-fix). La consolidación `isBeingWorkedOn` queda COMPLETA (6.º sitio). **Colisión de NUMERACIÓN** con runs paralelos c.281 ("pasado mañana") y c.282 ("entre lunes y viernes", parser), áreas ORTOGONALES, archivos DISJUNTOS de mi `GuardianCoach`: detectada en pre-commit `git fetch`; integrada vía ff-only sobre código DISJUNTO; AI_AUTONOMY reescrito sobre el remoto actualizado; renumerado a **c.283**. No force push, no reset --hard.
...
- **Próxima prioridad**: descubrimiento continuo (NO-parser). (i) Consolidación `isBeingWorkedOn` AHORA COMPLETA (6 sitios c.280+c.283); buscar otros dúos de código duplicado divergente (patrón c.266/c.280/c.283 — auditar predicados `isMissedStart`/`isImminentStart`/`isDueToday`/`isOverdue` dispersos y re-derivaciones "completada hoy"/"vencida"); (ii) consistencia entre superficies: otros tokens anidados tipo "pasado mañana" en asistente/búsqueda/contexto; (iii) áreas no-parser sin auditar (onboarding, navegación, accesibilidad, rendimiento, workers/backup con DAOs reales); (iv) verificar Android de c.270-c.283 cuando haya SDK; (v) parser residual sólo si aparece rendija P0/P1 clara. Re-fetch antes de implementar.
