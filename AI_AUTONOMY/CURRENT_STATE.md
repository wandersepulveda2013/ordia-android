...
- **Fecha (UTC)**: 2026-08-13 (ciclo 71; fix parser orden inverso "el mes que viene el N" agenda día N del mes siguiente; + c.72 fix parser "jueves que viene" dicho en jueves -> proxima semana (no HOY); + c.70 fix P0 crash parser día fuera de rango "el 0/99 de mes" + fix P1 "el 15 de agosto del 2027" no capturaba año "del" → fecha errónea 2026↔2027 y título "del 2027" huérfano; + c.69 feat resumen accionable: un toque pospone la tarea sugerida a mañana)
...
## Último trabajo — Ciclo 71: Parser — "jueves que viene" dicho en jueves → próxima semana (no HOY)

Fix P1 de integridad de agenda (`NaturalTaskParser`). La forma cotidiana **"reunión el jueves que
viene"** escrita un **jueves** caía en **HOY** en lugar de la próxima semana: la tarea se agendaba
el día equivocado (un compromiso explícitamente futuro perdido como evento de hoy). Mismo defecto
con "el próximo jueves", "lunes próximos", "sábado que viene", etc., cuando hoy ya era ese día.

**Causa raíz**: `weekdayPattern` capturaba el sufijo "que viene"/"próximo" como grupo **no
capturador**; el código de resolución nunca distinguía "el próximo jueves" del "jueves" suelto.
Cuando hoy coincidía con el día objetivo, `weekdaySameDayCandidate = true` + `nextWeekdayOrSame()`
(devuelve hoy si delta=0) dejaban la fecha en hoy.

**Solución (mínima, `NaturalTaskParser.kt`)**: en la rama `weekdayMatch`, `nextExplicit =
match.value.lowercase().contains("que viene") || match.value.lowercase().contains("próxim")` detecta el modificador de
"próxima ocurrencia" (tanto sufijo "que viene"/"próximos" como prefijo "próximo jueves" — el
prefijo ya está dentro de `match.value`). Con `nextExplicit` → `nextWeekday` estricto (+7 siempre)
y `weekdaySameDayCandidate = false`. Sin modificador, el día suelto ("el jueves a las 18" dicho en
jueves con hora futura) sigue pudiendo ser hoy — **no-regresión** del comportamiento introducido en
c.42 cont.2. Sin nueva pantalla/botón: misma potencia, menos pérdida de datos.

**Tests**: +7 en `NaturalTaskParserTest.kt` (juevesQueViene→+7; con hora; próximo jueves→+7;
jueves suelto con hora futura→hoy; viernes que viene→mañana; martes/miércoles que viene→próx.
semana; lunes próximos→próx. semana). **535 domain tests PASS** (26 clases — 528 c.70 + 7), smoke
25 OK. Rebase con run paralelo c.70 (P0/P1 parser mes-nombrado) sin conflicto de código, solo de
tests/docs resuelto conservando ambos conjuntos. **NO VERIFICADO**:
...
**semana+weekday** ("la semana que viene el lunes/viernes") sigue ABIERTA (BACKLOG): requiere helper
"start-of-next-week + weekday objetivo" distinto de `nextWeekday`.
...
**Siguiente (descubierto)**: semana+weekday "la semana que viene el lunes/viernes" sigue
ABIERTO (≠ `nextWeekday`; requiere start-of-next-week + weekday objetivo). Registrado en
BACKLOG.
