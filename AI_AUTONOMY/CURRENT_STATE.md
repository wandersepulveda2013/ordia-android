...
- **OPEN pendientes** (mayor impacto primero):
  ...
  - P3 residuos de título restantes ("a las 3.5" decimal-hour). (El residuo "fin de quincena"/"a fin de quincena" quedó RESUELTO c.274; "el primer" de cadencia precedente RESUELTO c.271; "cada \<unidad>" intervalo-1 RESUELTO c.271; "de \<mes>" de límites mensuales RESUELTO c.265; ordinal plurimensual precedente RESUELTO c.273.)
  - P2 decision `TaskStatus.CANCELLED` inalcanzable desde UI (requiere Android/UI).
  - Verificacion Android pendiente del fix c.264/c.266/c.268/c.270/c.271/c.272/c.273/c.274 cuando haya SDK (parser↔motor↔Room↔UI en runtime Android; build Compose de TodayScreen c.270).
- **Próxima prioridad**: descubrimiento continuo - (i) P2 "bisemanal"/"quincenal" como cadencia WEEKLY propia (probe ya identificó los fallos: freq=NONE e interval=1 — clase DISTINTA a c.273, no reusa el patrón mensual); (ii) seguir auditando el parser por paridad léxica residual (c.274 cerró la rendija "fin de quincena"; c.273 cerró la plurimensual de c.271; c.269 cerró la simetría captura↔búsqueda finde); (iii) seguir auditando motores no-parser por rendijas simétricas (c.268 cerró la de `DayPlanner`; c.270 cerró WhatNowReason build-break + auditó los 39 sitios `when`-on-enum sin `else` en UI no-domain -> todos exhaustivos); (iv) auditar OTROS orquestadores Android con copias duplicadas que puedan divergir (patrón simétrico al de c.266); (v) areas no-parser (onboarding, navegacion, accesibilidad, rendimiento, workers/backup con DAOs reales). Re-fetch antes de implementar.
