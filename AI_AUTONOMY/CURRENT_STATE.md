...
Bug **P1 (pérdida de datos silenciosa)**: `"último día del mes"` — la forma más común y
coloquial de decir "fin de mes" — no se parseaba. `endOfMonthPattern` solo reconocía
`fin`/`finales`, así `"entregar informe último día del mes"` caía en `dueAt=null` y la
frase quedaba como título → el vencimiento se olvidaba (sin recordatorio, invisible en
planificador/What Now). Pagos, rentas y cierres dichos en la forma más natural se perdían.

- **Síntoma (probe JVM, now=2026-07-29)**: `entregar informe último día del mes` →
  `due=null title='entregar informe último día del mes'` (BUG, debería 07-31). `fin de mes`
  → 07-31 (OK). La forma sin tilde `ultimo dia del mes` idem FAIL.
- **Causa raíz (doble)**:
  1. `endOfMonthPattern` solo listaba `fin(?:ales|es)?` como alternativa de "fin"; no
     incluía `últim[oa] día`.
  2. **Trampa sutil**: al añadir la alternativa `[uú]ltim[oa]\s+dí[ai]a`, el regex seguía
     sin casar porque el `\b` inicial (word boundary) es **ASCII** en la regex de Java/Kotlin
     por defecto → no reconoce un boundary antes de la `ú` (U+00FA, no-ASCII). Por eso
     `fin de mes` (empieza con `f` ASCII) sí casaba y `último día` no. Hubo que reemplazar
     `\b` por `(?<!\p{L})` (lookbehind Unicode-safe).
- **Solución mínima**:
  - `endOfMonthPattern`: `(?i)(?<!\p{L})(?:a\s+)?(?:fin(?:ales|es)?|[uú]ltim[oa]\s+d[ií]a)\s+(?:de\s+|del\s+)(?:pr[oó]xim[oa]\s+)?mes(?:\s+(?:que\s+viene|pr[oó]ximo|pr[oó]xima))?\b`.
    Reusa TODO el flujo de `fin de mes` (detección temprana, borrado, resolución, roll si
    hoy=último, combinar con hora explícita).
  - `monthBaseForBoundary`: rama "end" ahora `t.contains("fin") || t.contains("finales")
    || t.contains("últim") || t.contains("ultim")`. El modificador "que viene"/"próxim"
    ya se resuelve ANTES (línea `isNext`), así "último día del mes que viene" ancla al mes
    siguiente sin doble-desplazamiento.
...
- **Tests (TDD)**: +5 (`ultimoDiaDelMesParsesDueAtFinMesActual`,
  ...
  (`due=07-31 / 08-31`). Comando: `bash tools/run_domain_tests.sh` → **627 tests PASS**
  (27 clases — subida desde 622). Smoke: `bash tools/run_domain_checks.sh` → **25 OK**.
  Sin regresión (`fin de mes` y `el N del mes que viene` siguen OK).
