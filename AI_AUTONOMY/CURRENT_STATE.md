...
## Estado
- **Fecha (UTC)**: 2026-08-15. Rama `openhands/autonomous-ordia`. Este run es **c.292** (P0 datos (sagrados)/privacidad/seguridad — continuación directa de c.287/c.290/c.291): **cierre de una nueva desincronización de los dos gates de privacidad para credenciales cortas (PIN/NIP/contraseña).** Un SMS "tu clave temporal es 4821" o "tu pwd de acceso es ab12cd" pasaba el gate de contenido de `ConversationPrivacyPolicy` (persistencia de notificaciones): "clave temporal"/"clave bancaria" NO es "clave de acceso/seguridad/verificación" (única forma de "clave" reconocida), el `otpCode` no incluía "clave", y "pwd"/"nip" no estaban en la lista de palabras sensibles. El gate de lectura `ContextPrivacyFilter` SÍ los bloqueaba ("clave" en peludo; "pwd" ya listado) → MISMA asimetría de c.287/c.290: bloqueado al teclearse, PERSISTIDO al llegar como notificación. "nip" (PIN en español de México, sinónimo de "pin" que sí se bloqueaba) se fugaba en AMBOS gates. Probe JVM reproducible (clases reales) confirmó 3 desync + 1 gap compartido. Cierre simétrico: `ConversationPrivacyPolicy` añade `pwd`/`nip` al patrón bare-word y `clave` al `otpCode` (preciso — exige dígitos, no bloquea "la clave del éxito"); `ContextPrivacyFilter` añade `nip`. El guard de paridad c.291 se amplió con los 4 nuevos secretos. Base inicial `a101f77` (c.291). Entorno JVM (sin Android SDK): kotlinc 2.1.20, jars en `/tmp/libs`, OpenJDK 21.
- **Tests**: `bash tools/run_domain_tests.sh` -> **1858 PASS** (1857 c.291 + 1 c.292), 0 failures, 41 clases; `bash tools/run_domain_checks.sh` -> smoke 25 OK. Probe JVM confirmó los leaks PRE-fix (persist=false/read=true); POST-fix el nuevo test `blocksShortCredentialsThatEscapedNotificationsGate` y el guard de paridad extendido pasan. **NO VERIFICADO** gradle/lint/assemble/Android/UI/Room con DAOs reales (sin Android SDK).
- **Recientes (rama)**:
  - c.292 (este run): **P0 datos (sagrados)/privacidad/seguridad — credenciales cortas (PIN/NIP/contraseña) se persistían en texto plano aunque el gate de lectura las bloqueaba** - `ConversationPrivacyPolicy` (gate de persistencia de notificaciones) NO bloqueaba "clave temporal 4821"/"clave bancaria 9182" (no son "clave de acceso/seguridad/verificación"), "tu pwd de acceso es ab12cd" (pwd no listado) ni "tu nip del cajero es 4821" (nip no listado en NINGÚN gate). `ContextPrivacyFilter` (gate de lectura) SÍ bloqueaba los 3 primeros ("clave" en peludo, "pwd" listado) → MISMA asimetría de c.287/c.290. "nip" (PIN en español de México, sinónimo de "pin" que sí se bloqueaba) se fugaba en AMBOS gates. RESUELTO: cierre simétrico — `ConversationPrivacyPolicy` patrón 1 añade `pwd`+`nip`; `otpCode` añade `clave` (`\b(?:código|otp|verificación|clave)\D{0,20}\d{4,8}\b`, casa "clave temporal 4821" SIN bloquear "la clave del éxito" — exige dígitos); `ContextPrivacyFilter` patrón 1 añade `nip`. +1 test TDD (`blocksShortCredentialsThatEscapedNotificationsGate`, 5 leaks), +4 secretos al guard de paridad c.291, +3 inocentes anti-falso-positivo. Área ADYACENTE a c.287/c.290/c.291 (mismo tipo de desync, credenciales cortas). Sin nueva pantalla/botón.
...
- **Próxima prioridad**: descubrimiento continuo. (i) **Privacidad (continuación c.287→c.290→c.291→c.292)**: la auditoría de paridad entre los dos gates halló y cerró la familia de desync de credenciales (saldo/IBAN/seed c.287, claves privadas c.290, guard c.291, PIN/NIP/contraseña c.292). La causa raíz PERMANENTE sigue siendo **dos listas de patrones mantenidas a mano** — evaluar unificar la fuente de patrones en un objeto compartido en `domain` del que ambos gates lean (sin ciclo de paquetes: `context`→`conversations` ya existe vía `NotificationObservationPolicy`, así el compartido debe vivir en un paquete sin dependencias cíclicas). Mientras no se unifique, el guard de paridad c.291 (ampliado en c.292) atrapa nuevos desync; auditar si quedan OTROS contenidos sensibles (p. ej. números de INE/pasaporte/licencia — CIFs de identidad — que hoy escaparían a AMBOS gates); (ii) **Compromisos vencidos (continuación c.286→c.288→c.289)**: AHORA visibles en TRES superficies — asistente, nudge del guardián y tarjeta de insight. Queda `SummaryEngine.summarize` (resumen del día) por evaluar; ¿existe un worker push que AVISE al vencerse (no sólo consulta reactiva)?; (iii) textos del asistente que ignoran el `reason`/estado (familia c.285); (iv) consistencia entre superficies: predicados `isMissedStart`/`isImminentStart`/`isDueToday`/`isOverdue` dispersos; (v) áreas no-parser sin auditar (onboarding, navegación, accesibilidad, rendimiento, workers/backup con DAOs reales); (vi) verificar Android de c.270-c.292 cuando haya SDK; (vii) parser residual sólo si aparece rendija P0/P1 clara. Re-fetch antes de implementar.
