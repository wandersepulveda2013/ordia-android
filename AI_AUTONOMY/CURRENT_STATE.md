...
## Último trabajo — Ciclo 77: Resumen — el veredicto del día usa la ventana de jornada APRENDIDA (no la fija 9–18)

Mejora P1 de inteligencia/contexto (`SummaryEngine` ↔ `TodayScreen`). El veredicto del día
(`DayLoad`: LIGHT/ON_TRACK/FULL/OVERLOADED, c.65) comparaba los minutos restantes de hoy con el
tiempo libre hasta el **fin de jornada fijo 18:00** (inicio 9:00). Pero `LearningEngine` ya perfila
los horarios **reales** del usuario (percentil 10/90 de `completedAt`) y `DayPlanner` ya los usa
para agendar (c. de aprendizaje). El resumen ignoraba ese perfil → **plan y veredicto discrepaban**:
un usuario que trabaja hasta las 23:00 veía "OVERLOADED" a las 17:00 cuando le quedaban 6 h de
capacidad real; uno de jornada 6–14 veía "ON_TRACK" a las 13:00 cuando su día ya casi se acababa.
La tarjeta de hoy mentía para cualquier horario no estándar.

**Causa raíz**: `SummaryEngine.summarize`/`assessDayLoad` tenía `dayStartMinute=9*60` y
`dayEndMinute=18*60` **hardcoded** dentro del método privado, sin recibir el perfil aprendido. El
caller (`TodayScreen.kt:117`) llamaba `summarize(state.tasks, clockNow)` sin pasar perfil (aunque
`PlannerScreen` SÍ lo computaba con `LearningEngine.learn(...)` para `DayPlanner`).

**Solución (mínima, sin nueva pantalla/botón)**: `summarize` acepta ahora `dayStartMinute`/`dayEndMinute`
opcionales (defaults 9–18 = compatibilidad total con los 568 tests existentes) + una sobrecarga que
toma directamente un `LearningProfile?` (null → defaults). `assessDayLoad` recibe la ventana y la
usa en vez de las constantes hardcodeadas. `TodayScreen` calcula el perfil igual que `PlannerScreen`
(`if (preferences.learningEnabled) LearningEngine.learn(...) else null`) y lo pasa al `summarize`.
Ahora el veredicto y el planificador hablan el mismo idioma: la ventana es **fuente única de verdad**.
Sin aprendizaje → comportamiento idéntico al anterior (no-regresión).

**Tests**: +4 en `SummaryEngineTest.kt` (`dayLoad_usesLearnedWindow_lateSleeperNotOverloadedAt17`
jornada 9–23 a las 17:00 → ON_TRACK (era OVERLOADED con 9–18); `…earlyRiserOverloadedPastTheirEnd`
jornada 6–14 a las 13:00 → OVERLOADED (era ON_TRACK con 9–18, mentira); `…nullProfileFallsBackToDefaultWindow`
null ≡ defaults 9–18; `…learnedWindowDoesNotAffectCounts` la ventana solo cambia el veredicto, no los
conteos). **572 domain tests PASS** (`bash tools/run_domain_tests.sh`, 26 clases — 568 c.76 + 4),
smoke 25 OK (`tools/run_domain_checks.sh`). Sin regresión (todos los tests `dayLoad_*` y
`deferralSuggestion_*` previos intactos vía defaults).

**NO VERIFICADO**: gradle/lint/assemble/Android/UI/Room con DAOs reales; render real de la tarjeta
en la app (sin Android SDK). El cableado de `TodayScreen` sigue el patrón verificado de `PlannerScreen`.
