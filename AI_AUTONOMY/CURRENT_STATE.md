...
- **Próxima prioridad**: descubrimiento continuo — consolidación `isBeingWorkedOn` COMPLETA en los 5 sitios relevantes. Buscar (i) otros dúos de código duplicado divergentes (patrón c.266/c.53/c.86); (ii) áreas no-parser (onboarding, navegación, accesibilidad, rendimiento, workers/backup con DAOs reales — algunos marcar NO VERIFICADO); (iii) verificar Android de c.277-c.280 cuando haya SDK; (iv) parser residual sólo si aparece una rendija P0/P1 clara (paridad léxica muy cerrada). Re-fetch antes de implementar.
