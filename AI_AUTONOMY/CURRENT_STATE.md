...
## Estado
- **Fecha (UTC)**: 2026-08-15. Rama `openhands/autonomous-ordia`, HEAD inicial `76f00b0` (c.277 DayPlanner en-curso-primero). Este run **c.278** (NO-parser, distinto subsistema: `conversations`/CommitmentEngine) sobre base `76f00b0` síncrona con origin. Entorno JVM (sin Android SDK): kotlinc 2.1.20, jars en `/tmp/libs`, OpenJDK 21.
- **Tests**: `bash tools/run_domain_tests.sh` -> **1804 PASS** (1800 base c.277 + 4 c.278 CommitmentEngine "me encargo"/"me ocupo"), 0 failures, 40 clases; `bash tools/run_domain_checks.sh` -> smoke 25 OK. **NO VERIFICADO** gradle/lint/assemble/Android/UI/Room con DAOs reales (sin Android SDK).
- **Recientes (rama)**:
  - c.278 (este run): **P2 NO-parser CommitmentEngine no detectaba "me encargo"/"me ocupo" SIN "yo"** - run que audita un subsistema DISTINTO (conversaciones/compromisos) siguiendo la guía del supervisor de apartarse del parser y evitar loops en parser/recurrencia (c.271-c.276). `commitmentSignal` exigía `yo\s+me\s+encargo`; la respuesta canónica "me encargo de llamar al fontanero" (sin "yo", español pro-drop) generaba **0 drafts** (falso negativo → compromiso olvidado al importar chat). Lo mismo con "me ocupo de avisar a todos" (sinónimo). Asimetría: "yo me encargo" SÍ se detectaba; "me encargo" caía al vacío salvo que otra señal (comprar/reunión) coincidiera. RESUELTO: `commitmentSignal` fragmento `yo\s+me\s+encargo` → `(?:(?:yo\s+)?me\s+(?:encargo|ocupo))`: pronombre "yo" opcional + añade "me ocupo" (falso negativo verificado por probe, no sinónimo cosmético). Enrutado kind/owner existente sin cambios. Conservador: NO añadió "lo soluciono"/"te aviso" (riesgo de falso positivo, nicho). +4 tests TDD (2 RED→GREEN + 2 no-regresión).
...
- **Próxima prioridad**: descubrimiento continuo NO-parser — (i) auditar si `isRequest` (`avísame`/`recuérdame`/`pásame`) y otras señales de CommitmentEngine también exigen pronombres pro-drop omitidos en español (misma clase de falso negativo que c.278 cerró para `commitmentSignal`); (ii) auditar OTROS orquestadores Android con copias duplicadas que puedan divergir (patrón simétrico al de c.266); (iii) areas no-parser (onboarding, navegacion, accesibilidad, rendimiento, workers/backup con DAOs reales); (iv) parser residual sólo si aparece una rendija P0/P1 clara (la paridad léxica está muy cerrada: c.93/c.271/c.273/c.275/c.276). Re-fetch antes de implementar.
