...
## Estado
- **Fecha (UTC)**: 2026-08-15. Rama `openhands/autonomous-ordia`. Este run es **c.295** (P0 privacidad/seguridad/datos (sagrados) — continuación de la familia c.287→c.290→c.291→c.292→c.293 del desync de gates de privacidad, 5ª manifestación): **claves SSH públicas, API keys (sk-/sk_live_/sk_test_), AWS access key IDs (AKIA…) y JWTs (eyJ…) escapaban a AMBOS gates de privacidad y se persistían en texto plano.** A diferencia de c.287/c.290/c.292/c.293 (el gate de lectura bloqueaba pero el de persistencia dejaba escapar → el secreto se guardaba en texto plano), esta era una **RENDIJA COMPARTIDA**: ambos gates (`ConversationPrivacyPolicy` para persistencia de notificaciones Y `ContextPrivacyFilter` para lectura de contexto/IME) omitían estas 4 categorías. Así un SMS "tu API key es sk-…"/"AWS access key AKIA…"/"Bearer eyJ…JWT" pasaba ambos filtros y se persistía en texto plano en la BD de conversaciones Y se leía como contexto → fuga directa de credenciales de producción a BD/backup/ADB. RESUELTO: cierre simétrico de AMBOS gates con 4 patrones idénticos byte-a-byte — SSH (`ssh-(rsa|dsa|ecdsa|ed25519) <base64 20+>`), API keys (`sk[-_](live[-_]|test[-_])?[A-Za-z0-9]{20,}`), AWS (`(AKIA|ASIA|AGPA|AIDA|AROA|AIPA|ANPA|ANVA|ABIA|ACCA)[0-9A-Z]{16}`), JWT (`eyJ…\.eyJ…\.…`). +1 test TDD, +7 entradas al guard de paridad c.291, +2 inocentes. Sin nueva pantalla/botón, sin IA fingida (regex determinista). Base inicial `5f951c5` (c.293); el remoto avanzó durante el run: `5f951c5`→`3a4ba15` (**c.294** asistente "organiza mi día" nombra compromisos vencidos). Área ORTOGONAL, archivos DISJUNTOS (él `assistant/AssistantEngine`+`AssistantEngineTest`, yo `conversations/CommitmentEngine`+`context/ContextPrivacyFilter`+`CommitmentEngineTest`). Integración NO destructiva: `git stash` → `git pull --ff-only` (FF a `3a4ba15`) → `git stash pop` (auto-merge limpio, SIN conflictos: regiones disjuntas). No force push, no reset --hard, no STALE_RUN. Entorno JVM (sin Android SDK): kotlinc 2.1.20, jars en `/tmp/libs`, OpenJDK 21.
- **Tests**: `bash tools/run_domain_tests.sh` -> **1860 PASS** (1859 c.294 + 1 c.295), 0 failures, 41 clases (Time: 1.162s); `bash tools/run_domain_checks.sh` -> smoke 25 OK. Probe JVM PRE-fix confirmó 7/7 leaks (persist=false Y read=false); POST-fix 7/7 bloqueados por ambos gates, 0 falsos positivos en 12 inocentes de las nuevas categorías. **NO VERIFICADO** gradle/lint/assemble/Android/UI/Room con DAOs reales (sin Android SDK).
- **Recientes (rama)**:
  - c.295 (este run): **P0 privacidad/seguridad/datos (sagrados) — claves SSH públicas, API keys (sk-/sk_live_/sk_test_), AWS access key IDs (AKIA…) y JWTs (eyJ…) escapaban a AMBOS gates de privacidad y se persistían en texto plano** - continuación de la familia c.287→c.290→c.291→c.292→c.293 del desync de gates de privacidad, 5ª manifestación. A diferencia de las anteriores (el gate de lectura bloqueaba pero el de persistencia dejaba escapar), esta era una **RENDIJA COMPARTIDA**: ambos gates (`ConversationPrivacyPolicy` Y `ContextPrivacyFilter`) omitían estas 4 categorías de secretos de infraestructura/nube. Un SMS "tu API key es sk-…"/"AWS access key AKIAIOSFODNN7EXAMPLE"/"Bearer eyJ…JWT" pasaba ambos filtros y se persistía en texto plano en la BD de conversaciones Y se leía como contexto → fuga de credenciales de producción a BD/backup/ADB. RESUELTO: cierre simétrico de AMBOS gates con 4 patrones idénticos byte-a-byte — SSH (`ssh-(rsa|dsa|ecdsa|ed25519) <base64 20+>`), API keys (`(?i)sk[-_](live[-_]|test[-_])?[A-Za-z0-9]{20,}`), AWS (`(AKIA|ASIA|AGPA|AIDA|AROA|AIPA|ANPA|ANVA|ABIA|ACCA)[0-9A-Z]{16}`), JWT (`eyJ[A-Za-z0-9_-]{10,}\.[A-Za-z0-9_-]{10,}\.[A-Za-z0-9_-]{10,}`). +1 test TDD (`blocksCloudSecretsThatEscapedBothPrivacyGates`, 7 leaks), +7 entradas al guard de paridad c.291 (ahora afirma `persist==read==true` para los 7 — cierra la rendija compartida), +2 inocentes anti-falso-positivo ("mi ski de nieve", "el skateboard"). Probe JVM: PRE-fix 7/7 leaks (persist=false Y read=false), POST-fix 7/7 bloqueados por ambos gates, 0 falsos positivos en 12 inocentes de las nuevas categorías. Sin nueva pantalla/botón, sin IA fingida (regex determinista).
