...
- **Próxima prioridad**: descubrimiento continuo - (i) seguir auditando el parser por paridad léxica residual (c.275 cerró la rendija "bisemanal"; c.274 cerró la rendija "fin de quincena"; c.273 cerró la plurimensual de c.271; c.269 cerró la simetría captura↔búsqueda finde); revisar sinónimos de cadencia aún no cubiertos (p. ej. "decenal" — muy poco cotidiano, fuera de alcance); (ii) seguir auditando motores no-parser por rendijas simétricas (c.268 cerró la de `DayPlanner`; c.270 cerró WhatNowReason build-break + auditó los 39 sitios `when`-on-enum sin `else` en UI no-domain -> todos exhaustivos; GuardianEngine/GuardianCoach ya alineados con stale-inbox c.267/c.259); (iii) auditar OTROS orquestadores Android con copias duplicadas que puedan divergir (patrón simétrico al de c.266); (iv) areas no-parser (onboarding, navegacion, accesibilidad, rendimiento, workers/backup con DAOs reales). Re-fetch antes de implementar.
