...
## Estado
- **Fecha (UTC)**: 2026-08-15. Rama `openhands/autonomous-ordia`. Este run es **c.293** (P0 datos (sagrados)/privacidad/seguridad — continuación directa de c.287/c.290/c.291/c.292): **4ª manifestación del desync estructural entre los dos gates de privacidad.** Un IBAN alfanumérico PELADO ("Transfiere a GB82WEST12345698765432", sin la palabra "iban") y la frase inglesa "two factor"/"two-factor" se persistían en texto plano: `ConversationPrivacyPolicy` patrón 4 (`\d{13,19}`) sólo mira DÍGITOS → IBAN alfanumérico no casa; patrón 5 sólo bloquea la PALABRA "iban"; patrón 3 de 2FA sólo tenía "2fa", no "two.?factor". `ContextPrivacyFilter` SÍ los bloqueaba (IBAN estructural línea 35, "two.?factor" patrón 2) → MISMA asimetría: bloqueado al teclearse, PERSISTIDO al llegar como notificación. Probe JVM (clases reales) confirmó 3 desync. Cierre: `ConversationPrivacyPolicy` añade `two.?factor` al patrón 3 y un nuevo patrón 10 = IBAN estructural (byte-for-byte de `ContextPrivacyFilter` línea 35). Guard de paridad c.291 ampliado con 3 nuevos secretos. Base inicial `6aa64ed` (c.292). Entorno JVM (sin Android SDK): kotlinc 2.1.20, jars en `/tmp/libs`, OpenJDK 21.
- **Tests**: `bash tools/run_domain_tests.sh` -> **1859 PASS** (1858 c.292 + 1 c.293), 0 failures, 41 clases; `bash tools/run_domain_checks.sh` -> smoke 25 OK. Probe JVM confirmó los leaks PRE-fix (persist=false/read=true). **NO VERIFICADO** gradle/lint/assemble/Android/UI/Room con DAOs reales (sin Android SDK).
- **Recientes (rama)**:
  - c.293 (este run): **P0 datos (sagrados)/privacidad/seguridad — IBAN alfanumérico pelado y "two factor" inglés se persistían en texto plano** - `ConversationPrivacyPolicy` (gate de persistencia) NO bloqueaba "Transfiere a GB82WEST12345698765432" (IBAN estructural sin palabra "iban": patrón 4 `\d{13,19}` sólo dígitos → no casa alfanumérico; patrón 5 sólo la palabra) ni "your two factor code is 4821"/"two-factor authentication 9182" (patrón 3 sólo "2fa", no "two.?factor"). `ContextPrivacyFilter` (gate de lectura) SÍ los bloqueaba → MISMA asimetría de c.287/c.290/c.292. RESUELTO: `ConversationPrivacyPolicy` patrón 3 añade `two.?factor`; nuevo patrón 10 = `\b[A-Z]{2}\s?\d{2}(?:\s?[A-Z0-9]){11,30}\b` (IBAN estructural, byte-for-byte ISO 13616 de `ContextPrivacyFilter` línea 35). +1 test TDD (`blocksBareIbanAndTwoFactorThatEscapedNotificationsGate`, 4 leaks), +3 secretos al guard de paridad c.291. 4ª manifestación del desync — refuerza que el guard de paridad c.291 (ampliado en c.292/c.293) es estructuralmente necesario mientras no se unifique la fuente. Área ADYACENTE a c.287/c.290/c.292. Sin nueva pantalla/botón.
...
- **Próxima prioridad**: descubrimiento continuo. (i) **Privacidad (continuación c.287→c.290→c.291→c.292→c.293)**: la auditoría de paridad entre los dos gates halló y cerró la familia de desync de credenciales (saldo/IBAN/seed c.287, claves privadas c.290, guard c.291, PIN/NIP/contraseña c.292, IBAN alfanumérico pelado + "two factor" c.293 — 4ª manifestación). La causa raíz PERMANENTE sigue siendo **dos listas de patrones mantenidas a mano** — evaluar unificar la fuente de patrones en un objeto compartido en `domain` del que ambos gates lean (sin ciclo de paquetes: `context`→`conversations` ya existe vía `NotificationObservationPolicy`, así el compartido debe vivir en un paquete sin dependencias cíclicas). Mientras no se unifique, el guard de paridad c.291 (ampliado en c.292/c.293) atrapa nuevos desync; auditar si quedan OTROS contenidos sensibles (p. ej. números de INE/pasaporte/licencia — CIFs de identidad — que hoy escaparían a AMBOS gates); (ii) **Compromisos vencidos (continuación c.286→c.288→c.289)**: AHORA visibles en TRES superficies — asistente, nudge del guardián y tarjeta de insight. Queda `SummaryEngine.summarize` (resumen del día) por evaluar; ¿existe un worker push que AVISE al vencerse (no sólo consulta reactiva)?; (iii) textos del asistente que ignoran el `reason`/estado (familia c.285); (iv) consistencia entre superficies: predicados `isMissedStart`/`isImminentStart`/`isDueToday`/`isOverdue` dispersos; (v) áreas no-parser sin auditar (onboarding, navegación, accesibilidad, rendimiento, workers/backup con DAOs reales); (vi) verificar Android de c.270-c.293 cuando haya SDK; (vii) parser residual sólo si aparece rendija P0/P1 clara. Re-fetch antes de implementar.
