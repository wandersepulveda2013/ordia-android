...
## Estado
- **Fecha (UTC)**: 2026-08-15. Rama `openhands/autonomous-ordia`, HEAD `58d8934` (base c.266 remota; este run c.267 pendiente de commitear). Entorno JVM (sin Android SDK): kotlinc 2.1.20, jars en `/tmp/libs`, OpenJDK 21.
- **Tests**: `bash tools/run_domain_tests.sh` -> **1748 PASS** (1743 base c.266 + 5 míos c.267; el helper `RecurrenceSpawn` es cableado Android, fuera del harness de dominio), 0 failures, 40 clases; `bash tools/run_domain_checks.sh` -> smoke 25 OK. **NO VERIFICADO** gradle/lint/assemble/Android/UI/Room con DAOs reales (sin Android SDK).
- **Recientes (rama)**:
  - c.267 (este run): **feat(guardian) nudge diario nombra capturas arrinconadas** - cuando no hay atrasadas ni huecos perdidos pero hay una captura en bandeja (>=7 días, sin fecha, sin impulso real del día), el nudge la nombra ("«X» lleva 2 semanas en tu bandeja sin fecha. Hazla hoy, agéndala o quítala.") en vez del mensaje genérico "completa una tarea breve". Recuperación de tareas olvidadas en la superficie existente (sin nueva pantalla). Adicional: fix de consistencia de zona - `staleInboxAction` usaba `ZoneId.systemDefault()` mientras el resto del snapshot usaba el `zoneId` pasado; ahora se propaga `zoneId` (snapshot -> suggestedAction -> staleInboxAction), correcto en cualquier zona (verificado con systemDefault=UTC+14). +5 tests (RED confirmado: al revertir la rama, el test `nombraCapturaArrinconada` falla).
  - c.266 (run paralelo): **P1 fuente-única recurrencia+notificación perdía el desglose** - completar una recurrente CON subtareas desde el RECORDATORIO no clonaba el checklist (sólo la app lo hacía, c.223/c.236). RESUELTO: nuevo orquestador `RecurrenceSpawn.spawnNextOccurrence` (top-level en `com.ordia.app.reminders`) unifica los 4 caminos spawn (2 ViewModel + 2 ReminderActionReceiver) en una sola fuente: `nextOccurrence`→`add`→`schedule`→clonar subtareas→re-enlazar etiquetas. El path de la notificación GANA el clonado que antes perdía. Composición de reglas puras ya verificadas; orquestador Android = NO VERIFICADO JVM.
...
- **Proxima prioridad**: descubrimiento continuo - (i) seguir auditando motores no-parser (`WhatNowEngine`/`SummaryEngine`/`SearchEngine`/`DayPlanner`) por rendijas simetricas (GuardianEngine/GuardianCoach ya alineados con stale-inbox c.267/c.259); (ii) areas no-parser (onboarding, navegacion, accesibilidad, rendimiento, workers/backup con DAOs reales); (iii) auditar OTROS orquestadores Android con copias duplicadas que puedan divergir (patrón simétrico al de c.266). Re-fetch antes de implementar.
