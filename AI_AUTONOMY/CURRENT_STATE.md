...
## Estado
- **Fecha (UTC)**: 2026-08-15. Rama `openhands/autonomous-ordia`. Este run es **c.286** (NO-parser, área "asistente / evitar olvidos / cuarta clase de olvido"): el asistente ("¿qué olvidé?"/"vencidas"/"resume conversación") **IGNORABA los compromisos vencidos extraídos de conversaciones** — la cuarta clase de olvido de Ordía (promesa con `dueAt` vencido, `PENDING`, sin convertir en tarea) era invisible para la superficie de recuperación que existe precisamente para no dejar olvidos pasar. `AssistantEngine.answer` recibía `commitments` PERO sólo los usaba para los compromisos sin fecha (resumen de conversación); un compromiso vencido se callaba aunque el usuario preguntara "¿qué olvidé?". RESUELTO: nuevo `CommitmentRules.isOverduePending`/`overduePendingSorted` (predicado determinista: `PENDING && dueAt != null && dueAt < now`, orden por `dueAt` ascendente, más atrasado primero) consumido por 4 ramas del asistente (forgotten/vencidas/missed-start-tail/resume-conversación): nombra el compromiso vencido y ofrece `OPEN_CONVERSATIONS` (guiar, no ejecutar: convertir/descartar es decisión del usuario), y si también hay tarea vencida lo añade como cola informativa para no callarlo. Base inicial `0d6037b` (c.285 what-now "Sigue con"/"Te quedan"); colisión de base con c.285 (misma familia `AssistantEngine*`, área ADYACENTE no idéntica): detectada en pre-commit `git fetch`; remoto avanzó `16dbebd`→`0d6037b` mientras trabajaba; integración NO destructiva vía stash→ff-only→pop (sin force push/reset --hard): `.kt` regiones DISJUNTAS (c.285 rama what-now l.71-90, yo rama forgotten/vencidas/missed-start l.110-195 + helpers l.405-428) → auto-merge limpio sin conflictos. Entorno JVM (sin Android SDK): kotlinc 2.1.20, jars en `/tmp/libs`, OpenJDK 21.
- **Tests**: `bash tools/run_domain_tests.sh` -> **1842 PASS** (1829 base c.285 + 13 c.286: 8 CommitmentRules + 5 AssistantEngine), 0 failures, 41 clases; `bash tools/run_domain_checks.sh` -> smoke 25 OK. **NO VERIFICADO** gradle/lint/assemble/Android/UI/Room con DAOs reales (sin Android SDK).
- **Recientes (rama)**:
  - c.286 (este run): **P1 evitar olvidos / datos (sagrados) / IA honesta — asistente ignoraba la cuarta clase de olvido: compromisos vencidos de conversaciones** - `AssistantEngine.answer` recibía la lista de `commitments` pero SÓLO la usaba para los compromisos SIN fecha en el resumen de "resume conversación". Un compromiso con `dueAt` vencido y `reviewStatus==PENDING` (una promesa que se pasó de plazo sin convertir en tarea ni descartar) —la cuarta clase de olvido de Ordía, distinta de las tres de tareas (vencida / hueco incumplido / captura arrinconada) porque un compromiso no es tarea hasta que se convierte— era **invisible** para la superficie de recuperación: si el usuario preguntaba "¿qué olvidé?" o "vencidas" y no había tareas vencidas, el asistente respondía "No tienes tareas vencidas." **mintiendo por omisión** sobre una promesa vencida. Y si SÍ había tarea vencida, nombraba la tarea y callaba el compromiso. RESUELTO: nuevo objeto de dominio `CommitmentRules` con `isOverduePending(commitment, now)` (`PENDING && dueAt != null && dueAt < now`) y `overduePendingSorted(commitments, now)` (orden por `dueAt` ascendente, más atrasado primero; orden estable a igual plazo) —predicado determinista, no IA fingida. `AssistantEngine.answer` calcula los compromisos vencidos y los expone en 4 ramas: (1) forgotten sin tarea vencida → nombra el compromiso más atrasado + `OPEN_CONVERSATIONS`; (2) "vencidas" sin tarea vencida → igual; (3) forgotten/missed-start CON tarea vencida → añade cola informativa ("Además, tienes N compromisos vencidos...") para no callarlo; (4) "resume conversación" → distingue compromisos vencidos de los pendientes a futuro. La acción es `OPEN_CONVERSATIONS` (guiar, no ejecutar a ciegas: convertir en tarea o descartar es decisión del usuario sobre su propia promesa, no una replanificación automática). Sin nueva pantalla/botón —el botón "Conversaciones" ya existe y `AssistantAction.OPEN_CONVERSATIONS` ya se consumía. +13 tests TDD (8 CommitmentRules RED→GREEN cubriendo vencido/null/futuro/no-PENDING/dueAt==now/orden/estabilidad/vacío; 5 AssistantEngine: recuperación pura de compromiso vencido, query "vencidas", cola cuando hay tarea vencida, resume-conversación marca vencido, guard anti-falso-positivo con compromiso futuro/revisado). Guard `overdueIntent_doesNotFlagStaleInboxAsOverdue` confirma que una captura arrinconada SIN dueAt no se reporta falsamente como vencida. `bash tools/run_domain_tests.sh` → **1842 PASS** (1829 base c.285 + 13 c.286), 0 failures, 41 clases; `bash tools/run_domain_checks.sh` → smoke 25 OK. Sin regresión (tests existentes de compromisos/asistente intactos). **NO VERIFICADO** gradle/lint/assemble/Android/UI/Room con DAOs reales (sin Android SDK); integración `AssistantEngine`↔`AssistantScreen`/`ConversationsScreen` en runtime Android (dominio puro `CommitmentRules` + `AssistantEngine.answer` SÍ verificados en JVM). **Colisión de base** con run paralelo c.285 (what-now "Sigue con"): mismo archivo `AssistantEngine.kt`/`AssistantEngineTest.kt` PERO regiones DISJUNTAS (c.285 rama what-now l.71-90, yo rama forgotten/vencidas/missed-start l.110-195 + helpers l.405-428): integrada vía stash→ff-only→pop (no destructivo, sin force push/reset --hard): auto-merge limpio SIN conflictos. Re-verificado tras integración: 1842 PASS.
...
- **OPEN pendientes** (mayor impacto primero):
  ...
  - Verificacion Android pendiente del fix c.286 (compromisos vencidos en el asistente: CommitmentRules↔AssistantEngine.answer↔AssistantScreen/ConversationsScreen en runtime Android; ¿"¿qué olvidé?" ya nombra un compromiso vencido de una conversación y ofrece abrir Conversaciones? ¿la acción OPEN_CONVERSATIONS lleva a la pantalla correcta? ¿GuardianEngine/GuardianCoach/SummaryEngine también mencionan compromisos vencidos o siguen callándolos — ver próxima prioridad (i)?) cuando haya SDK.
- **Próxima prioridad**: descubrimiento continuo (NO-parser). (i) Compromisos vencidos AHORA visibles al asistente (c.286); extender esa recuperación a las DEMÁS superficies de olvido que aún callan un compromiso vencido: `GuardianEngine`/`GuardianCoach` (tarjeta de insight) y `SummaryEngine` (resumen del día) — ¿mencionan compromisos vencidos o sólo tareas?; además, ¿existe un recordatorio/worker que avise de un compromiso vencido (no sólo la consulta reactiva "¿qué olvidé?")?; (ii) textos del asistente que ignoran el `reason`/estado (familia c.285) — otras ramas de `AssistantEngine.answer` que componen frases sin mirar el motivo; (iii) consistencia entre superficies: predicados `isMissedStart`/`isImminentStart`/`isDueToday`/`isOverdue` y re-derivación "vencida" dispersos; tokens anidados tipo "pasado mañana" en otras superficies; (iv) áreas no-parser sin auditar (onboarding, navegación, accesibilidad, rendimiento, workers/backup con DAOs reales); (v) verificar Android de c.270-c.286 cuando haya SDK; (vi) parser residual sólo si aparece rendija P0/P1 clara. Re-fetch antes de implementar.
