...
## Estado
- **Fecha (UTC)**: 2026-08-15. Rama `openhands/autonomous-ordia`. Este run es **c.284** (NO-parser, área "orquestadores de tareas"/"fuente única de verdad"): **unifica "completadas hoy" en las TRES superficies que lo re-derivaban** — `GuardianEngine.snapshot.completedToday` (ánimo/energía), `SummaryEngine.summarize.completedToday` (badge "Completadas hoy") y `GuardianCoach.insight` (tarjeta de insight) reimplementaban el predicado inline y divergieron. GuardianEngine NO chequeaba `completed` (un `completedAt` hoy + `completed=false` inflaba el ánimo: IA deshonesta); SummaryEngine contaba archivadas y canceladas-completadas-hoy; GuardianCoach ya era correcto. Nuevo `TaskRules.completedTodayCount` (canónico, idéntico a `completedRootCount` + filtro temporal) consumido por las 3. Esto era la "próxima prioridad (i)" que c.283 dejó marcada: "auditar ... re-derivaciones 'completada hoy'/'vencida'". Base inicial `fbcc86a` (c.283), sincronizada sin colisión. Entorno JVM (sin Android SDK): kotlinc 2.1.20, jars en `/tmp/libs`, OpenJDK 21.
- **Tests**: `bash tools/run_domain_tests.sh` -> **1827 PASS** (1817 c.283 + 10 c.284), 0 failures, 40 clases; `bash tools/run_domain_checks.sh` -> smoke 25 OK. **NO VERIFICADO** gradle/lint/assemble/Android/UI/Room con DAOs reales (sin Android SDK).
- **Recientes (rama)**:
  - c.284 (este run): **P2 consistencia entre superficies / inteligencia honesta / datos (sagrados) — orquestadores "completadas hoy" (familia c.280/c.283, señal "completada hoy" en vez de "en curso")** - TRES superficies re-derivaban "completadas hoy" inline y divergieron. `GuardianEngine.snapshot.completedToday` (ánimo/energía/metas diarias del guardián) contaba tareas con `completedAt==hoy` PERO SIN chequear `completed` ni `status!=CANCELLED` → un dato inconsistente (`completedAt` hoy + `completed=false`, p. ej. tras backup-restore o camino futuro) inflaba el ánimo sobre actividad inexistente (IA deshonesta: el guardián decía "vas genial, N hoy" sobre actividad que no ocurrió). `SummaryEngine.summarize.completedToday` (badge "Completadas hoy") contaba `completed` hoy PERO incluía archivadas y canceladas-completadas-hoy (un logro descartado/archivado no es logro activo). `GuardianCoach.insight` (tarjeta de insight) YA filtraba correctamente (`!archived && status!=CANCELLED`) — era la fuente de verdad de facto pero por coincidencia, no por estructura. RESUELTO: nuevo predicado canónico `TaskRules.completedTodayCount(tasks, now, zone)` — idéntico a `completedRootCount` (raíz + `completed` + `!archived` + `status!=CANCELLED`) más el filtro temporal `completedAt==hoy` (zona del usuario, vía `DateRules.toLocalDate`). Las TRES superficies lo consumen. Sin nueva pantalla/botón, sin IA fingida (predicado determinista compartido). +10 tests TDD (RED→GREEN, RED confirmado empíricamente revirtiendo CADA superficie a su forma inline vieja: GuardianEngine `expected:<0> but was:<1>` para `completed=false` y para CANCELLED; SummaryEngine `expected:<0> but was:<1>` para CANCELLED y archivada).
...
- **Próxima prioridad**: descubrimiento continuo (NO-parser). (i) Re-derivación "completadas hoy" AHORA unificada (c.284); continuar patrón c.266/c.280/c.283/c.284 — auditar predicados `isMissedStart`/`isImminentStart`/`isDueToday`/`isOverdue` y re-derivación "vencida" dispersos entre superficies; (ii) consistencia entre superficies: otros tokens anidados tipo "pasado mañana" en asistente/búsqueda/contexto; (iii) áreas no-parser sin auditar (onboarding, navegación, accesibilidad, rendimiento, workers/backup con DAOs reales); (iv) verificar Android de c.270-c.284 cuando haya SDK; (v) parser residual sólo si aparece rendija P0/P1 clara. Re-fetch antes de implementar.
