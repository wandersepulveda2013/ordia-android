...
## Estado
- **Fecha (UTC)**: 2026-08-15. Rama `openhands/autonomous-ordia`, HEAD inicial `76f00b0` (c.277 DayPlanner en-curso-primero). Este run abarca **c.278 + c.279** (NO-parser, subsistema `conversations`/CommitmentEngine): c.278 cerró un falso NEGATIVO pro-drop ("me encargo"/"me ocupo" sin "yo"); c.279 cerró un falso POSITIVO de negación ("no te llamo"/"no me encargo") descubierto en la auditoría de seguimiento de c.278. Base `76f00b0` síncrona con origin; c.278 commit `ac3a9c2` pusheado; c.279 sobre `ac3a9c2`. Entorno JVM (sin Android SDK): kotlinc 2.1.20, jars en `/tmp/libs`, OpenJDK 21.
- **Tests**: `bash tools/run_domain_tests.sh` -> **1806 PASS** (1800 base c.277 + 4 c.278 + 2 c.279), 0 failures, 40 clases; `bash tools/run_domain_checks.sh` -> smoke 25 OK. **NO VERIFICADO** gradle/lint/assemble/Android/UI/Room con DAOs reales (sin Android SDK).
- **Recientes (rama)**:
  - c.279 (este run): **P2 NO-parser CommitmentEngine marcaba NEGATIVAS directas como compromiso** - auditoría de seguimiento de c.278. `commitmentSignal.containsMatchIn(text)` ignoraba el "no" precedente, así **"no te llamo hasta mañana"**/**"no me encargo de eso"**/**"no lo hago yo"**/**"no voy a ir"**/**"no debo olvidarlo"**/**"no tengo que hacerlo hoy"** generaban 1 draft (falso positivo → IA deshonesta: el usuario dijo NO y Ordía proponía la tarea). Probe JVM pre-fix: 6/6 negativas → 1 draft. RESUELTO: nuevo `hasUnnegatedCommitment(text)` recorre cada match de `commitmentSignal` y descarta las ocurrencias precedidas por `\bno\s+`; hay compromiso solo si AL MENOS una ocurrencia no está negada. Así "no te llamo"→0 pero "no tengo tiempo, lo hago mañana"→1 (compromiso real preservado, "no" lejos del verbo). **NO se aplica a `isRequest`/`isReminder`** (allí la negación es idiomática y POSITIVA: "no olvides"=recuérdame). Trade-off honesto: c.278 ganó recall, c.279 ganó precision. +2 tests TDD (1 RED→GREEN + 1 no-regresión). Clase de bug PRE-EXISTENTE, no introducida por c.278.
...
- **Próxima prioridad**: descubrimiento continuo — **cambiar de subsistema NO-parser** para no saturar `conversations` (ya 2 ciclos: c.278 + c.279). Candidatos: (i) auditar OTROS orquestadores Android con copias duplicadas que puedan divergir (patrón simétrico al de c.266); (ii) areas no-parser (onboarding, navegación, accesibilidad, rendimiento, workers/backup con DAOs reales — algunos marcar NO VERIFICADO); (iii) verificar Android de los fixes c.277/c.278/c.279 cuando haya SDK; (iv) parser residual sólo si aparece una rendija P0/P1 clara (la paridad léxica está muy cerrada: c.93/c.271/c.273/c.275/c.276). Re-fetch antes de implementar.
