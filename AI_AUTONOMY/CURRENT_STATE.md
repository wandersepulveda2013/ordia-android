...
## Estado
- **Fecha (UTC)**: 2026-08-15. Rama `openhands/autonomous-ordia`. Este run es **c.285** (NO-parser, área "asistente/inteligencia honesta"): el asistente "¿qué hago ahora?" decía **"Empieza por «X»: ya está en curso. Estimo 25 minutos"** cuando lo sugerido era una tarea EN CURSO ([WhatNowReason.IN_PROGRESS_NOW]) — contradictorio ("empezar" lo que está en curso) e impreciso (la duración planificada completa cuenta el tramo ya vivido). RESUELTO: "Sigue con" + "Te quedan N minutos" (lo que FALTA, vía `remainingPlanMinutes`) cuando hay ventana activa; "Sigue con" + "Estimo" cuando está marcada en curso a mano (sin `startAt`, elapsed desconocido — no se simula). Base inicial `fbcc86a` (c.283); durante el run el remoto avanzó a `16dbebd` (**c.284**, "completadas hoy" fuente única, área ORTOGONAL, archivos DISJUNTOS de mi `AssistantEngine`). Integración NO destructiva vía stash→ff-only→pop (sin force push/reset --hard): `.kt` disjuntos sin conflictos. Cedido c.284 al run paralelo; este run renumerado a **c.285**. Entorno JVM (sin Android SDK): kotlinc 2.1.20, jars en `/tmp/libs`, OpenJDK 21.
- **Tests**: `bash tools/run_domain_tests.sh` -> **1829 PASS** (1827 c.284 + 2 c.285), 0 failures, 40 clases; `bash tools/run_domain_checks.sh` -> smoke 25 OK. **NO VERIFICADO** gradle/lint/assemble/Android/UI/Room con DAOs reales (sin Android SDK).
- **Recientes (rama)**:
  - c.285 (este run): **P2 inteligencia honesta / consistencia — asistente "¿qué hago ahora?" mentía sobre tareas en curso** - la rama what-now de `AssistantEngine` componía SIEMPRE `"Empieza por «X»: $why. Estimo $minutes minutos."` sin mirar el `reason`. Si `WhatNowEngine.suggest` devolvía `IN_PROGRESS_NOW` (tarea con ventana activa `isInProgressNow`, o marcada `IN_PROGRESS` a mano), el `why` era "ya está en curso" → la frase quedaba **"Empieza por «Borrador»: ya está en curso. Estimo 25 minutos."**: (1) contradicción ("empieza" lo que está en curso) y (2) el tiempo miente — `plannedDuration` devuelve la duración planificada completa aunque el tramo `startAt..ahora` ya se haya vivido; el usuario con 15 min hechos de una tarea de 25 leía "Estimo 25" (debería "Te quedan 10"). RESUELTO: distingue por `reason` y por `isInProgressNow`. `lead = if (inProgress) "Sigue con" else "Empieza por"` (la acción correcta según el estado); `timePhrase = if (elapsedKnown) "Te quedan $minutes" else "Estimo $minutes"` donde `minutes = if (elapsedKnown) remainingPlanMinutes else plannedDuration`. Sólo afirma "Te quedan" con ventana activa (elapsed conocido); una tarea marcada en curso a mano (sin `startAt`) conserva "Estimo" porque no sabemos cuánto se ha trabajado — no se simulan datos. Paridad conceptual con c.277 (DayPlanner reservaba `remainingPlanMinutes` para lo en curso) y c.280/c.283 (predicado canónico `isBeingWorkedOn`). Sin nueva pantalla/botón, sin IA fingida (predicados deterministas existentes). +2 tests TDD (`whatNow_inProgressWindow_saysContinueNotStart_andShowsRemainingTime`: RED pre-fix decía "Empieza por"+"Estimo 25" sobre una ventana 15/25min; `whatNow_inProgressManualFlag_saysContinueButKeepsEstimate`: "Sigue con"+"Estimo" para IN_PROGRESS a mano, NO "Te quedan"). No toca la tarjeta What Now de `TodayScreen` (ésta muestra título+etiqueta "En curso ahora mismo", sin duración ni "Empieza por" → sin contradicción ahí). AUDITORÍA: los otros 3 consumidores de WhatNowEngine (`AutomationActionPlanner`, `SearchEngine`, `TaskRules`) sólo lo referencian en comentarios, sin texto contradictorio.
...
- **Próxima prioridad**: descubrimiento continuo (NO-parser). (i) Re-derivación "completadas hoy" AHORA unificada (c.284); continuar patrón c.266/c.280/c.283/c.284/c.285 — auditar predicados `isMissedStart`/`isImminentStart`/`isDueToday`/`isOverdue` y re-derivación "vencida" dispersos entre superficies; (ii) consistencia entre superficies: otros tokens anidados tipo "pasado mañana" en asistente/búsqueda/contexto; textos del asistente que ignora el `reason`/estado (familia c.285 — p. ej. otras ramas de `AssistantEngine.answer` que componen frases sin mirar el motivo); (iii) áreas no-parser sin auditar (onboarding, navegación, accesibilidad, rendimiento, workers/backup con DAOs reales); (iv) verificar Android de c.270-c.285 cuando haya SDK; (v) parser residual sólo si aparece rendija P0/P1 clara. Re-fetch antes de implementar.
