...
## Estado
- **Fecha (UTC)**: 2026-08-15. Rama `openhands/autonomous-ordia`. Este run es **c.288** (P1 evitar olvidos / consistencia entre superficies / IA honesta, área "guardián nudge / evitar olvidos / recuperación de información importante"): el nudge del guardián (`GuardianEngine.snapshot.suggestedAction`) recuperaba los TRES olvidos de tareas (atrasada / hueco incumplido / captura arrinconada) PERO callaba el CUARTO — el compromiso vencido extraído de una conversación (`CommitmentRules.isOverduePending` de c.286). c.286 cerró la rendija en el asistente (superficie a demanda "¿qué olvidé?"); este run la cierra en el guardián (superficie PROACTIVA: el nudge que el usuario ve sin preguntar). RESUELTO: `GuardianEngine.snapshot` recibe `commitments` y `suggestedAction` añade rama `overdueCommitmentAction` (nombra el compromiso más atrasado + "Revísalo en Conversaciones") más cola informativa `withCommitmentTail` cuando ya nombró una tarea olvidada (no lo calla, no doble señalización). **Cableado UI** (novedad estructural vs c.286): `OrdiaUiState` añade `commitments`; la cadena `combine` del `OrdiaViewModel` lo injerta vía `.combine(commitments) { state, c -> state.copy(commitments = c) }`; `GuardianScreen`/`MoreScreen` pasan `commitments = state.commitments` a `snapshot()`. Base inicial `9af2ecd` (c.286); durante el run el remoto avanzó a `2d3e6f9` (**c.287**, privacidad notificaciones, área ORTOGONAL, archivos DISJUNTOS de mi `GuardianEngine`/`OrdiaViewModel`/`GuardianScreen`/`MoreScreen`/`GuardianEngineTest`). Integración NO destructiva vía stash→ff-only→pop (sin force push/reset --hard): `.kt` disjuntos sin conflictos. Cedido c.287 al run paralelo; este run renumerado a **c.288**. Entorno JVM (sin Android SDK): kotlinc 2.1.20, jars en `/tmp/libs`, OpenJDK 21.
- **Tests**: `bash tools/run_domain_tests.sh` -> **1850 PASS** (1845 c.287 + 5 c.288), 0 failures, 41 clases; `bash tools/run_domain_checks.sh` -> smoke 25 OK. **NO VERIFICADO** gradle/lint/assemble/Android/UI/Room con DAOs reales (sin Android SDK).
- **Recientes (rama)**:
  - c.288 (este run): **P1 evitar olvidos / consistencia entre superficies — guardián callaba la cuarta clase de olvido: compromisos vencidos de conversaciones** - `GuardianEngine.snapshot.suggestedAction` recuperaba los tres olvidos de tareas (atrasada / hueco incumplido / captura arrinconada) PERO callaba el compromiso vencido extraído de una conversación (`CommitmentRules.isOverduePending`: `dueAt` pasado, `PENDING`, sin convertir en tarea). c.286 cerró la rendija en el asistente (a demanda "¿qué olvidé?"); este run la cierra en el guardián (PROACTIVO: el nudge que el usuario ve sin preguntar). Así un día sin vencidas ni huecos pasados pero con una promesa vencida recibía el nudge genérico "Completa una tarea breve" — mintiendo por omisión; y si SÍ había tarea vencida, nombraba la tarea y callaba el compromiso. RESUELTO: `snapshot` recibe `commitments: List<CommitmentEntity> = emptyList()` (backward-compatible); `suggestedAction` añade rama `overdueCommitments.isNotEmpty() -> overdueCommitmentAction(...)` DESPUÉS de overdue/missed (lo accionable en una tarea ya capturada manda; el compromiso no es tarea hasta convertirse → acción es guiar "Revísalo en Conversaciones", paridad con c.286) + cola `withCommitmentTail` cuando ya nombró una tarea olvidada. **Cableado UI**: `OrdiaUiState.commitments`; `OrdiaViewModel.uiState` vía `.combine(commitments) { state, c -> state.copy(commitments = c) }`; `GuardianScreen`/`MoreScreen` pasan `commitments = state.commitments`. Sin nueva pantalla/botón, sin IA fingida (predicado `CommitmentRules` determinista de c.286). +5 tests TDD. Área ADYACENTE a c.286 (mismo "4º olvido", superficie distinta: guardián proactivo vs asistente a demanda).
...
- **Próxima prioridad**: descubrimiento continuo (NO-parser). (i) **Compromisos vencidos (continuación c.286→c.288)**: AHORA visibles al asistente (c.286, a demanda) y al nudge del guardián (c.288, proactivo). Quedan DOS superficies de olvido que aún callan un compromiso vencido: `GuardianCoach.insight` (la tarjeta de insight permanente — la superficie MÁS visible) y `SummaryEngine.summarize` (el resumen del día). Evaluar si deben mencionarlo (paridad completa del "4º olvido" en TODAS las superficies); además, ¿existe un recordatorio/worker que AVISE de un compromiso vencido (no sólo consulta reactiva/nudge — una notificación push proactiva al vencerse)?; (ii) **Privacidad (continuación c.287)**: `ConversationPrivacyPolicy` (notificaciones/conversación) y `ContextPrivacyFilter` (contexto/IME) cubren lo mismo en lo financiero/cripto PERO SIGUEN SIENDO DOS LISTAS — evaluar unificar la fuente de patrones sin crear ciclo de paquetes (p. ej. objeto compartido en `domain` del que ambos lean); auditar si quedan OTROS contenidos sensibles que escapen al gate de notificaciones (private keys 64-hex de `ContextPrivacyFilter`, ¿deberían estar en notificaciones?); (iii) textos del asistente que ignoran el `reason`/estado (familia c.285) — otras ramas de `AssistantEngine.answer` que componen frases sin mirar el motivo; (iv) consistencia entre superficies: predicados `isMissedStart`/`isImminentStart`/`isDueToday`/`isOverdue` y re-derivación "vencida" dispersos; tokens anidados tipo "pasado mañana" en otras superficies; (v) áreas no-parser sin auditar (onboarding, navegación, accesibilidad, rendimiento, workers/backup con DAOs reales); (vi) verificar Android de c.270-c.288 cuando haya SDK; (vii) parser residual sólo si aparece rendija P0/P1 clara. Re-fetch antes de implementar.
