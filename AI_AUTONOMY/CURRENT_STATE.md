...
## Estado
- **Fecha (UTC)**: 2026-08-15. Rama `openhands/autonomous-ordia`. Este run es **c.282** (parser recurrencia "entre X y Y" / conector "entre"). Base inicial `645baf9` (c.279); durante el run el remoto avanzó DOS veces por runs paralelos: `2017135` (c.280 orquestadores `isBeingWorkedOn`, archivos DISJUNTOS) y `95d141e` (c.281 "pasado mañana" asistente+busqueda, archivos DISJUNTOS de mi parser). Integracion NO destructiva via `fetch+rebase` (sin force push/reset --hard): `.kt` auto-mergiaron limpios (regiones disjuntas); `.md` resueltos a mano CONSERVANDO los 3 runs (c.280, c.281, c.282). Cedido c.280 y c.281 a los runs paralelos que llegaron primero al remoto; este run renumerado a c.282 para evitar colision de etiquetas. Entorno JVM (sin Android SDK): kotlinc 2.1.20 (`/tmp/kotlinc-home/kotlinc/bin/kotlinc`), jars en `/tmp/libs`, OpenJDK 21.
- **Tests**: `bash tools/run_domain_tests.sh` -> **1816 PASS** (1811 base c.280 + 3 c.281 "pasado manana" + 2 c.282 mios), 0 failures, 40 clases; `bash tools/run_domain_checks.sh` -> smoke 25 OK. **NO VERIFICADO** gradle/lint/assemble/Android/UI/Room con DAOs reales (sin Android SDK).
- **Recientes (rama)**:
  - c.282 (este run): **P1 parser "entre lunes y viernes" mutilaba la rutina y "entre \<día> y \<día>" dejaba el conector como residuo del título** - auditoría del parser (paridad léxica + limpieza de captura). "Gimnasio entre lunes y viernes" (forma habitual de pedir la semana laboral, simétrica a "de lunes a viernes") NO casaba `weekdayRangePattern` (sólo "de/los lunes a viernes") y caía a `dayListPattern`, que la leía como **lista de DOS días sueltos {lunes, viernes}** → `recDays="1,5"` (rutina mutilada en silencio: ¡sólo lunes y viernes, no la semana!) Y dejaba "entre" como residuo del título ("Gimnasio entre"). Misma clase de bug en cualquier "entre \<día> y \<día>": "futbol entre sabado y domingo"→título "futbol **entre**" (días 6,7 correctos como lista, pero conector sucio). RESUELTO (dos cambios mínimos, sin nueva pantalla/botón, sin IA fingida — regex determinista): (1) `weekdayRangePattern` añade la alternativa `entre\s+lunes\s+(?:a|y)\s+viernes` → "entre lunes y viernes"/"entre lunes a viernes" se resuelven como el rango Lun-Vie WEEKLY 1-5 con título limpio, simétrico a "de lunes a viernes"; (2) `dayListPattern` antepone un conector inicial opcional NO capturador `(?:entre\s+)?` → consume "entre " cuando precede a una lista real de días ("entre sabado y domingo", "entre martes y jueves") sin alterar índices de grupo (hasPrefix=[1], artículo=[2], días=[3]); el rango Lun-Vie ya se resuelve arriba, así que aquí sólo llegan listas reales. No introduce falsos positivos: frases con "entre" sin día ("decidir entre opcion a y opcion b") NO casan (no hay nombre de día) → quedan como texto libre. +2 tests TDD (RED→GREEN vía probe pre-fix). Clase de bug PRE-EXISTENTE.
  - c.281 (`95d141e`, run paralelo integrado): **P2 consistencia entre superficies — "pasado mañana" resolvía a MAÑANA en asistente + búsqueda**. "pasado mañana" contiene el token "mañana": `AssistantEngine.agendaAnswer` y `SearchEngine.detectDateScope` caían en su rama "mañana" → el usuario que pregunta "¿qué tengo pasado mañana?" recibía la agenda de MAÑANA, y la búsqueda "pasado mañana" devolvía las de mañana (mentira de agenda/búsqueda → posible olvido de lo que vence pasado mañana). Asimetría: una TERCERA superficie (`ContextIntentEngine` l.417) YA lo manejaba bien con guarda `!contains("pasado mañana")`. RESUELTO en las dos superficies faltantes: asistente añade rama `"pasado manana" in query` antes de `"manana"`; SearchEngine añade `DateScope.DAY_AFTER_TOMORROW` + rama `detectDateScope`/`anchorMatchesScope`. +3 tests TDD (RED→GREEN). Área ORTOGONAL a c.282 (archivos DISJUNTOS: AssistantEngine/SearchEngine vs NaturalTaskParser).
...
- **OPEN pendientes** (mayor impacto primero):
  ...
  - Verificacion Android pendiente del fix c.281 ("entre lunes y viernes" / "entre \<día> y \<día>": parser↔motor↔Room↔UI de recurrencia en runtime Android) cuando haya SDK.
...
- **Próxima prioridad**: descubrimiento continuo (NO-parser). (i) Seguir auditando consistencia entre superficies: otros tokens anidados tipo "pasado mañana" (p. ej. "anteayer"↔"ayer", "esta semana que viene"↔"esta semana") en asistente/búsqueda/contexto; (ii) otros dúos de código duplicado divergente (patrón c.266/c.280 isBeingWorkedOn); (iii) áreas no-parser sin auditar (onboarding, navegación, accesibilidad, rendimiento, workers/backup con DAOs reales); (iv) verificar Android de c.270-c.281 cuando haya SDK; (v) parser residual sólo si aparece rendija P0/P1 clara. Re-fetch antes de implementar.
