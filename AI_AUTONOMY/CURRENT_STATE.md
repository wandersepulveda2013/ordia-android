...
## Estado
- **Fecha (UTC)**: 2026-08-15. Rama `openhands/autonomous-ordia`. Este run es **c.294** (P1 evitar olvidos / consistencia entre superficies / IA honesta — continuación de la familia c.286→c.288→c.289 del "4.º olvido"): **la superficie de planificación del asistente ("organiza mi día") silenciaba los compromisos vencidos de conversaciones.** c.286 cerró la rendija en el asistente a demanda ("¿qué olvidé?"/"vencidas"); c.288 en el nudge del guardián (proactivo); c.289 en la tarjeta de insight (permanente). Quedaba la superficie de PLANIFICACIÓN —"organiza mi día"—, justo donde más importa saberlo antes de decidir el plan. `AssistantEngine.answer` ya calculaba `overdueCommitments` (c.286) y los exponía en 3 ramas PERO en "organiza mi día" decía "Hay N pendientes y M vencidas" callando la promesa vencida. RESUELTO: anexa `overdueCommitmentTail(overdueCommitments)` (helper existente de c.286) a esa rama — cola informativa, acción primaria sigue OPEN_PLANNER (no doble señalización: la promesa no se convierte a ciegas, se recuerda para que el usuario decida). Sin nueva pantalla/botón, sin IA fingida (predicado `CommitmentRules.overduePendingSorted` determinista de c.286). +2 tests TDD. Base inicial `a101f77` (c.291); el remoto avanzó durante el run: `6aa64ed` (**c.292** privacidad credenciales cortas) → `5f951c5` (**c.293** privacidad IBAN alfanumérico pelado + "two factor"). Ambos área ORTOGONAL, archivos DISJUNTOS (él `conversations/CommitmentEngine`+`context/ContextPrivacyFilter`+`CommitmentEngineTest`, yo `assistant/AssistantEngine`+`AssistantEngineTest`). Integración NO destructiva en DOS pasos: `git stash` → `git pull --ff-only` → `git stash pop` (auto-merge limpio en código `.kt`); conflictos sólo en AI_AUTONOMY `.md` (colisión de numeración: ambos añadimos entrada "c.293" al inicio) resueltos a mano renumerando mi trabajo a **c.294**. No force push, no reset --hard, no STALE_RUN. Entorno JVM (sin Android SDK): kotlinc 2.1.20, jars en `/tmp/libs`, OpenJDK 21.
- **Tests**: `bash tools/run_domain_tests.sh` -> **1861 PASS** (1859 c.293 + 2 c.294), 0 failures, 41 clases; `bash tools/run_domain_checks.sh` -> smoke 25 OK. RED del bug confirmado pre-fix (sin el tail, "organiza mi día" no contenía "compromiso vencido"). **NO VERIFICADO** gradle/lint/assemble/Android/UI/Room con DAOs reales (sin Android SDK).
- **Recientes (rama)**:
  - c.294 (este run): **P1 evitar olvidos / consistencia entre superficies / IA honesta — la superficie de planificación del asistente ("organiza mi día") silenciaba los compromisos vencidos de conversaciones** - continuación de la familia c.286→c.288→c.289 del "4.º olvido". `AssistantEngine.answer` ya calculaba `overdueCommitments` (c.286) y los exponía en 3 superficies (asistente a demanda, nudge guardián, tarjeta permanente) PERO en la rama "organiza mi día" respondía "Hay N pendientes y M vencidas. Puedo preparar un plan…" callando la promesa vencida — justo en la superficie donde más importa saberlo ANTES de decidir el plan. Así un día con 0 vencidas pero 1 compromiso vencido decía "0 vencidas" ocultando el olvido. RESUELTO: anexa `overdueCommitmentTail(overdueCommitments)` (helper existente de c.286) — "Además, tienes N compromiso(s) vencido(s) de conversaciones por convertir en tarea." Cola informativa; la acción primaria sigue OPEN_PLANNER (no doble señalización: la promesa no se convierte a ciegas, se recuerda para que el usuario decida — paridad con la cola de c.286/c.288). +2 tests TDD (`organizeDay_surfacesOverdueCommitmentAsTail` + guard anti-falso-positivo `organizeDay_doesNotMentionCommitmentWhenNoneOverdue`). Área ADYACENTE a c.286 (mismo "4º olvido", superficie de planificación del asistente), región DISJUNTA (rama `organiza mi dia` vs `¿qué olvidé?`/`vencidas`/`resume conversación`). Sin nueva pantalla/botón, sin IA fingida.
