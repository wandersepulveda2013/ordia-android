...
## Estado
- **Fecha (UTC)**: 2026-08-15. Rama `openhands/autonomous-ordia`. Este run es **c.287** (P0 privacidad, área "privacidad/datos sagrados/seguridad"): el gate de notificaciones (`NotificationObservationPolicy` → `ConversationPrivacyPolicy`) era MÁS DÉBIL que el gate de contexto (`ContextPrivacyFilter`) y dejaba escapar contenido financiero/cripto en texto plano (saldo, estado de cuenta, transferencia, frase semilla, IBAN como palabra) que llegaba vía SMS/mensajería — el MISMO texto era bloqueado al teclearse pero PERSISTIDO al llegar como notificación. RESUELTO: `ConversationPrivacyPolicy` elevado a paridad financiera/cripto con `ContextPrivacyFilter` (3 grupos: identificadores de cuenta, seed cripto, contexto financiero) conservando `otpCode`; el contenido bancario de paquetes no bancarios ya se bloquea en el gate de notificaciones. Base inicial `0d6037b` (c.285); durante el run el remoto avanzó a `9af2ecd` (**c.286**, asistente recupera compromisos vencidos de conversaciones, área ORTOGONAL, archivos DISJUNTOS de mi `CommitmentEngine`/`NotificationObservationPolicyTest`). Integración NO destructiva vía stash→ff-only→pop (sin force push/reset --hard): `.kt` disjuntos sin conflictos. Cedido c.286 al run paralelo; este run renumerado a **c.287**. Entorno JVM (sin Android SDK): kotlinc 2.1.20, jars en `/tmp/libs`, OpenJDK 21.
- **Tests**: `bash tools/run_domain_tests.sh` -> **1845 PASS** (1842 c.286 + 3 c.287), 0 failures, 41 clases; `bash tools/run_domain_checks.sh` -> smoke 25 OK. **NO VERIFICADO** gradle/lint/assemble/Android/UI/Room con DAOs reales (sin Android SDK).
- **Recientes (rama)**:
  - c.287 (este run): **P0 privacidad — gate de notificaciones dejaba escapar contenido financiero/cripto** - `ConversationPrivacyPolicy` (usado por `NotificationObservationPolicy`, `ConversationsScreen`, `CommitmentEngine`) cubría password/pin/cvv/otp/código de verificación + card 13-19 dígitos, PERO NO `saldo`/`estado de cuenta`/`transferencia`/`depósito`/`retiro`, ni `iban`/`clabe`/`número de cuenta` como palabra, ni `frase semilla`/`recovery phrase`/`mnemonic`. `ContextPrivacyFilter` (usado por contexto/IME) SÍ los cubría. Un SMS/notificación bancario llega desde la app de mensajería (paquete NO bancario, pasa el filtro de paquete), así dependía del filtro de contenido — que dejaba escapar y PERSISTIR saldos, estados de cuenta y seed phrases cripto. RESUELTO: 3 grupos añadidos a `ConversationPrivacyPolicy.sensitivePatterns` (identificadores de cuenta, seed cripto, contexto financiero) alcanzando paridad con `ContextPrivacyFilter`; `otpCode` conservado (las notificaciones son la fuente típica de 2FA). Se descartó reemplazar por `ContextPrivacyFilter` (perdería `otpCode`, regresaría fuga OTP) y delegar (crearía ciclo de paquetes `conversations`↔`context`). NO se añadieron content categories sexuales/violentas a notificaciones (P2 moderación, no P0 datos; riesgo falso positivo). +3 tests TDD (`blocksFinancialAndCryptoContentThatEscapedNotificationsGate` 8 leaks, `innocentConversationTextIsNotBlockedByPrivacyGate` 4 inocentes no falso positivo, `financialContentFromNonBankingPackagesIsRejected` path completo del gate). Probe re-run POST-fix confirma las 3 fugas ahora bloqueadas. Sin nueva pantalla/botón, sin IA fingida (regex deterministas). Área ORTOGONAL a c.286 (privacidad vs asistente), archivos DISJUNTOS.
  - c.286 (`9af2ecd`, run paralelo integrado): **P1 evitar olvidos / datos (sagrados) / IA honesta — asistente ignoraba la cuarta clase de olvido: compromisos vencidos de conversaciones** - `AssistantEngine.answer` recibía la lista de `commitments` pero SÓLO la usaba para los compromisos SIN fecha en el resumen de "resume conversación". Un compromiso con `dueAt` vencido y `reviewStatus==PENDING` (una promesa que se pasó de plazo sin convertir en tarea ni descartar) era **invisible** para la superficie de recuperación. RESUELTO: nuevo `CommitmentRules.isOverduePending`/`overduePendingSorted` consumido en 4 ramas del asistente. +13 tests TDD. Área ORTOGONAL a c.287 (asistente vs privacidad), archivos DISJUNTOS.
...
- **Próxima prioridad**: descubrimiento continuo (NO-parser). (i) **Privacidad (continuación c.287)**: ahora `ConversationPrivacyPolicy` (notificaciones/conversación) y `ContextPrivacyFilter` (contexto/IME) cubren lo mismo en lo financiero/cripto PERO SIGUEN SIENDO DOS LISTAS — evaluar unificar la fuente de patrones sin crear ciclo de paquetes (p. ej. objeto compartido en `domain` del que ambos lean); auditar si quedan OTROS contenidos sensibles que escapen al gate de notificaciones (private keys 64-hex de `ContextPrivacyFilter`, ¿deberían estar en notificaciones?); auditar `ContextIntentEngine` y otros consumidores de privacidad por divergencia residual; (ii) Compromisos vencidos AHORA visibles al asistente (c.286); extender esa recuperación a las DEMÁS superficies de olvido que aún callan un compromiso vencido: `GuardianEngine`/`GuardianCoach` (tarjeta de insight) y `SummaryEngine` (resumen del día) — ¿mencionan compromisos vencidos o sólo tareas?; además, ¿existe un recordatorio/worker que avise de un compromiso vencido (no sólo la consulta reactiva "¿qué olvidé?")?; (iii) textos del asistente que ignoran el `reason`/estado (familia c.285) — otras ramas de `AssistantEngine.answer` que componen frases sin mirar el motivo; (iv) consistencia entre superficies: predicados `isMissedStart`/`isImminentStart`/`isDueToday`/`isOverdue` y re-derivación "vencida" dispersos; tokens anidados tipo "pasado mañana" en otras superficies; (v) áreas no-parser sin auditar (onboarding, navegación, accesibilidad, rendimiento, workers/backup con DAOs reales); (vi) verificar Android de c.270-c.287 cuando haya SDK; (vii) parser residual sólo si aparece rendija P0/P1 clara. Re-fetch antes de implementar.
