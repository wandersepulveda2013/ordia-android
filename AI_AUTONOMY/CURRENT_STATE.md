...
## Estado
- **Fecha (UTC)**: 2026-08-15. Rama `openhands/autonomous-ordia`, HEAD inicial `4237df7` (c.273 parser ordinal plurimensual precedente, sincronizado vía fetch+ff-only). Run paralelo c.272 (`7c7b910`, parser "este mes/año") + c.273 (`4237df7`, plurimensual) + c.274 (`62a75c7`, quincena "fin de"/"a fin de" residuo de título, rebaseado sobre c.273, código DISJUNTO) integrados; este run **c.275** (bisemanal/bisemanalmente → cadencia WEEKLY/2) rebaseado sobre c.274 (código DISJUNTO: `parseRecurrence`/`detectWeekInterval` l.~3022/3370/3441 vs `quincenaPattern` l.~536; `.md` resueltos a mano conservando AMBOS). Entorno JVM (sin Android SDK): kotlinc 2.1.20, jars en `/tmp/libs`, OpenJDK 21.
- **Tests**: `bash tools/run_domain_tests.sh` -> **1788 PASS** (1777 base c.273 + 6 c.274 quincena "fin de" + 5 c.275 bisemanal), 0 failures, 40 clases; `bash tools/run_domain_checks.sh` -> smoke 25 OK. **NO VERIFICADO** gradle/lint/assemble/Android/UI/Room con DAOs reales (sin Android SDK).
- **Recientes (rama)**:
  - c.275 (este run): **P1 parser "bisemanal"/"bisemanalmente" no se reconocía como cadencia** - continuación del cierre de paridad léxica de c.93/c.271/c.273. "reunión bisemanal"/"pago bisemanal"/"reunión bisemanalmente" caían a `rec=NONE interval=1 dueAt=null` (rutina olvidada: invisible en What Now/planificador, recordatorio jamás disparaba) Y "bisemanal" quedaba como residuo literal en el título. Además "bisemanal los lunes" disparaba la rama de lista semanal con `interval=1` (cada semana, el DOBLE de frecuente de lo pedido) y dejaba el título sucio. RESUELTO: `detectWeekInterval()` reconoce `\bbisemanal(?:mente)?\b`→interval 2 (forma con días "bisemanal los lunes"→WEEKLY/2+days, simétrico a "cada dos semanas los lunes"); nuevo bloque `\bbisemanal(?:mente)?\b` → `WEEKLY interval=2` (plusWeeks(2)=14 días, idéntico a "cada dos semanas", análogo WEEKLY de "bimestral" MONTHLY/2), colocado antes de `fixedPatterns` (que sólo admite interval=1). `recurrenceAdjectiveLeakPattern` añade `bisemanal(?:mente)?` para limpiar el residuo en casos de anclaje contradictorio ("pago bisemanal el 15 de cada mes"→MONTHLY, título "pago"). Distinto de "quincenal" (DAILY/15): una quincena son 15 días, no 14. +5 tests TDD (RED→GREEN vía probe pre-fix).
