...
## Estado
- **Fecha (UTC)**: 2026-08-15. Rama `openhands/autonomous-ordia`. Este run es **c.291** (P0 datos (sagrados)/seguridad — continuidad estructural de c.287/c.290): **creación de guard de regresión de paridad entre los dos gates de privacidad.** c.290 cerró una fuga de claves privadas cripto en el gate de **persistencia** (`ConversationPrivacyPolicy`) que el gate de **lectura** (`ContextPrivacyFilter`) sí bloqueaba; c.287 cerró la misma asimetría para saldo/IBAN/seed phrase. Ambos bugs comparten **causa raíz estructural**: dos listas de patrones mantenidas a mano en gates distintos se desincronizaron, y como **no existía test de paridad**, el desync llegó a producción. c.291 añade `privacyGatesStayInSyncOnSecrets` (en `CommitmentEngineTest.kt`) que afirma el invariante más estrecho y crítico: para cada secreto de las categorías que **nunca** deben persistirse, ambos gates coinciden y bloquean. Verificado (demo RED reproducible): al eliminar los patrones de clave privada de `ConversationPrivacyPolicy`, el nuevo test **falla** junto con `blocksPrivateKeysThatEscapedNotificationsGate` (c.290) — el guard es real, no cosmético. Restaurado → 1857 PASS. Sin nueva pantalla/botón. Base inicial `de89b8d` (c.290). Entorno JVM (sin Android SDK): kotlinc 2.1.20, jars en `/tmp/libs`, OpenJDK 21.
- **Tests**: `bash tools/run_domain_tests.sh` -> **1857 PASS** (1856 c.290 + 1 c.291), 0 failures, 41 clases; `bash tools/run_domain_checks.sh` -> smoke 25 OK. Demo RED de c.291 verificada localmente (no persistida): remover los 2 patrones de `ConversationPrivacyPolicy` rompe 2 tests. **NO VERIFICADO** gradle/lint/assemble/Android/UI/Room con DAOs reales (sin Android SDK).
- **Recientes (rama)**:
  - c.291 (este run): **P0 datos (sagrados)/seguridad — guard de regresión de paridad entre los dos gates de privacidad (previene recurrencia estructural de c.287/c.290)** - c.287 y c.290 fueron el MISMO bug dos veces: dos listas de patrones mantenidas a mano en gates distintos (`ConversationPrivacyPolicy` para persistencia de notificaciones vs `ContextPrivacyFilter` para lectura de contexto/IME) se desincronizaron, y el secreto (saldo/IBAN/seed en c.287; claves privadas cripto en c.290) se **persistía en texto plano** aunque el gate de lectura SÍ lo bloqueaba. Causa raíz: **no existía test de paridad**, así el desync llegaba a producción. RESUELTO: nuevo test `privacyGatesStayInSyncOnSecrets` (en `CommitmentEngineTest.kt`) que afirma el invariante más estrecho y crítico — para cada secreto de las categorías que NUNCA deben persistirse (claves cripto 64-hex/0x/PEM, seed phrase, saldo/estado de cuenta, contraseña, código de seguridad), ambos gates coinciden (`assertEquals`) Y bloquean (`assertTrue`). No busca paridad total (el gate de lectura bloquea además adultos/violencia/política, que NO deben bloquear la persistencia) — sólo el invariante de secretos. Demo RED reproducible: al eliminar los 2 patrones de clave privada de `ConversationPrivacyPolicy`, el nuevo test falla junto con `blocksPrivateKeysThatEscapedNotificationsGate` (c.290) — el guard es real, no cosmético. +1 test TDD. Área ORTOGONAL a c.290 (misma causa raíz, capa de protección de tests vs capa de patrones). Sin nueva pantalla/botón.
...
- **Próxima prioridad**: descubrimiento continuo. (i) **Compromisos vencidos (continuación c.286→c.288→c.289)**: AHORA visibles en TRES superficies — asistente (c.286, a demanda), nudge del guardián (c.288, proactivo) y tarjeta de insight (c.289, permanente). Queda UNA superficie que aún calla un compromiso vencido: `SummaryEngine.summarize` (el resumen del día). Evaluar si debe mencionarlo (paridad completa del "4º olvido" en TODAS las superficies); además, ¿existe un recordatorio/worker que AVISE de un compromiso vencido (no sólo consulta reactiva/nudge/tarjeta — una notificación push proactiva al vencerse)?; (ii) **Privacidad (continuación c.287)**: `ConversationPrivacyPolicy` (notificaciones/conversación) y `ContextPrivacyFilter` (contexto/IME) cubren lo mismo en lo financiero/cripto PERO SIGUEN SIENDO DOS LISTAS — evaluar unificar la fuente de patrones sin crear ciclo de paquetes (p. ej. objeto compartido en `domain` del que ambos lean); auditar si quedan OTROS contenidos sensibles que escapen al gate de notificaciones (private keys 64-hex de `ContextPrivacyFilter`, ¿deberían estar en notificaciones?); (iii) textos del asistente que ignoran el `reason`/estado (familia c.285) — otras ramas de `AssistantEngine.answer` que componen frases sin mirar el motivo; (iv) consistencia entre superficies: predicados `isMissedStart`/`isImminentStart`/`isDueToday`/`isOverdue` y re-derivación "vencida" dispersos; tokens anidados tipo "pasado mañana" en otras superficies; (v) áreas no-parser sin auditar (onboarding, navegación, accesibilidad, rendimiento, workers/backup con DAOs reales); (vi) verificar Android de c.270-c.289 cuando haya SDK; (vii) parser residual sólo si aparece rendija P0/P1 clara. Re-fetch antes de implementar.
