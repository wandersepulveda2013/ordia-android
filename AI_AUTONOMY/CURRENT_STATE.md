...
## Último trabajo — Ciclo 99: Parser — números escritos > 30 y compuestos ("cuarenta y cinco minutos") no se parseaban → tarea SIN vencimiento (P1 captura/agenda olvidada)

Bug **P1 (captura/agenda olvidada)**: las cantidades escritas > 30 y la forma compuesta
estándar del español no se reconocían, así que la tarea nacía **sin `dueAt`**:

| Entrada | Antes (bug) | Ahora |
|---|---|---|
| `llamar en cuarenta y cinco minutos` | dueAt=null, dur=5 ("cinco" rezuma) | dueAt=+45min, título "llamar" |
| `reunión en cuarenta minutos` | dueAt=null, dur=null | dueAt=+40min |
| `llamar en sesenta minutos` | dueAt=null, dur=null | dueAt=+60min |
| `en cincuenta minutos` | dueAt=null, dur=null | dueAt=+50min |
| `en treinta y cinco minutos` | dueAt=null, dur=5 | dueAt=+35min |
| `en veinticinco minutos` | dueAt=null, dur=null | dueAt=+25min |
| `trabajar cuarenta y cinco minutos` | dur=null (duración perdida) | dur=45 |
| `vuelo recuérdame cuarenta y cinco minutos antes` | reminderOffset=null | reminderOffset=45 |

La persona decía "llamar en cuarenta y cinco minutos" y Ordía agendaba una tarea **sin
fecha** → invisible en What Now/planificador, sin recordatorio posible. Asimetría
flagrante con los dígitos ("en 45 minutos" sí funcionaba desde siempre).

**Causa raíz (duplicación + acotamiento)**: las listas de palabras admitidas estaban
acotadas a **1-30** y **duplicadas** en 6 patrones (`relativePattern`,
`compoundFractionalRelativePattern`, `multiQuarterRelativePattern`, `agoPattern`,
`writtenAmountPattern` —compartido por `reminderPatterns`/`writtenDurationPattern`— y un
`writtenNumberGroup` local de `parseRecurrence`). Además `parseWrittenNumber` solo mapeaba
1-21 y 30. Así "cuarenta"/"cincuenta"/"sesenta" no casaban, y la forma compuesta
"cuarenta y cinco" tampoco (la unidad baja "cinco" rezumaba a veces como duración
espuria). El bug podía aparecer en un patrón y no en otro por la duplicación.

**Solución (mínima, `NaturalTaskParser.kt`, sin nueva pantalla/botón — "menos interfaz,
más potencia")**:
1. Un **ÚNICO** `writtenNumberGroup` (fragmento regex, 1-99) declarado al inicio del
   `object` y reutilizado por TODOS los patrones que antes duplicaban la lista. Cubre:
   - formas compuestas `(decena) y (unidad)` (31-99, p.ej. "treinta y cinco") y
     `veinte y (unidad)` (21-29), puestas ANTES que las decenas sueltas para que
     "cuarenta y cinco" case entero (no solo "cuarenta");
   - las palabras únicas veintidós…veintinueve (+ sin tilde) y veintiuno;
   - decenas sueltas treinta…noventa y unidades un…nueve.
   Se **elimina** la lista duplicada de `parseRecurrence` (ahora usa la propiedad
   compartida). Menos código + consistencia: el bug **no puede reaparecer** en un
   patrón olvidado.
2. `parseWrittenNumber` reescrito con un `wordToNumber` map (1-99) + resolución del
   compuesto "decena y unidad" (left ∈ decenas redondas {20,30,…,90}, right ∈ 1..9 →
   l+r). Mantiene dígitos y "un par de"→2.

Lógica local honesta (mapeo de token + aritmética de decena, sin random ni IA falsa).
Simétrico a los dígitos: "en cincuenta minutos" == "en 50 minutos".

**Tests (TDD)**: probe JVM inicial confirmó RED en los 8 casos; convertidos en 7 tests
permanentes en `NaturalTaskParserTest.kt`: `writtenCompoundNumberRelativeParsesDueAt`,
`writtenTensRelativeParsesDueAt`, `writtenCompoundNoLeakLowUnitAsDuration`,
`writtenTwentiesSingleWordParsesDueAt`, `writtenCompoundTensParsesDueAt`,
`writtenCompoundDurationParsesMinutes`, `writtenTensReminderParsesOffset`. Comando:
`bash tools/run_domain_tests.sh` → **682 tests PASS** (28 clases — 675 c.97 + 7 propios).
Smoke: `bash tools/run_domain_checks.sh` → **25 OK**. Sin regresión.
...
**Próxima prioridad**: descubrimiento continuo — "cada rato"/"de vez en cuando"
(recurrencia vaga, requiere anti-falso-positivo); números > 99 escritos ("ciento
veinticinco minutos", raro pero posible — evaluar demanda real antes); detección de
compromisos en notas; auditoría de captura/búsqueda/What Now para nuevas oportunidades.


## Último trabajo — Ciclo 98: Parser — "el día N" (día de mes suelto con la palabra "día") no se parseaba → cita olvidada o título corrupto (P1 captura/agenda)

Bug **P1 (integridad de captura / cita olvidada)**: `"el día N"` —la forma más cotidiana
en español de fijar un día de mes suelto ("reunión el día 30", "pago el día 15 de cada
mes", "reunión el día 1 de enero")— NO se parseaba. `dayOfMonthPattern` solo casaba
"el N" (sin la palabra "día"), así que `"reunión el día 30"` caía a `dueAt=null`:
**pérdida silenciosa de la cita** (sin recordatorio, invisible en What Now/planificador).
Peor aún: en `"pago el día 15 de cada mes"` / `"reunión el día 1 de enero"`, las ramas
`monthlyDayPattern`/`monthNamePattern` SÍ capturaban la fecha pero NO consumían la
palabra "día" → título basura `"reunión el día"`. Asimetría flagrante: `"el 30"`
funcionaba (c.68/c.70), pero la forma con la palabra `"día"` —la más natural en español—
no. El usuario creía agendar y la cita nacía sin vencimiento o con título incomprensible.

**Causa raíz**: cuatro patrones (`dayOfMonthPattern`, `nextMonthDayPattern`,
`monthlyDayPattern`, `monthNamePattern`) que capturan un día numérico del mes solo lo
esperaban precedido del artículo "el", NO de la palabra "día" ("el día 30" = "el" + "día"
+ "30"). Sin consumir "día", el grupo de día no casaba → `dueAt=null`; y cuando otra rama
capturaba la fecha sin consumir "día", esta palabra quedaba como residuo en el título.

**Solución (mínima, `NaturalTaskParser.kt`, sin nueva pantalla/botón — "menos interfaz,
más potencia")**: los cuatro patrones añaden `(?:d[ií]a\s+)?` opcional antes del grupo de
día (casa con y sin tilde, límites `\b` intactos). Reusa TODO el flujo existente:
resolución de fecha (hoy/próximo mes con clamp al último día válido), combinación con hora
explícita, recurrencia mensual, roll si el día ya pasó. Así `"el día N"` resuelve a la
misma fecha que `"el N"` y la frase completa se consume → título limpio. Lógica local
honesta (mapeo de token, sin random ni modelo simulado). Retrocompatible (sin cambios de
firma pública). La forma sin "día" sigue funcionando idéntica (no-regresión: `"reunión
el 30"` → 30/07 OK).

**Tests**: +6 en `NaturalTaskParserTest.kt`: `parsesElDiaStandaloneDayOfMonth` ("Reunión
el día 30" → 30/07 + título "Reunión"), `parsesElDiaWithoutTilde` ("Reunión el dia 3" →
03/08, sin tilde), `elDiaNextMonthResolvesToNextMonth` ("Entregar el día 1 del mes que
viene" → 01/08), `elDiaWithExplicitHour` ("Reunión el día 15 a las 10" → 15/08 10:00),
`elDiaMonthlyRecurrenceCleanTitle` ("Pago el día 15 de cada mes" → MONTHLY + 15/08 +
título "Pago"), `elDiaMonthNameCleanTitle` ("Reunión el día 1 de enero" → 01/01/2027 +
título "Reunión"). Los 3 primeros fallaron antes de corregir las fechas esperadas (el
`now` del fixture es 29-jul, no 12-ago: el 30 todavía no llegó → 30/07, el 3 ya pasó →
03/08, "mes que viene"=agosto → 01/08). `bash tools/run_domain_tests.sh` → **681 PASS**
(28 clases, 675 previos + 6); `bash tools/run_domain_checks.sh` → smoke 25 OK. Probe JVM
amplio (28 casos) confirmó sin regresión: las formas con "día" (con/sin tilde, con mes que
viene, con hora, mensual, con mes nombrado) todas verdes; las formas sin "día" ("el 30"/
"el 15 del mes que viene"/"el 15 de cada mes"/"el 15 de agosto") intactas.

**NO VERIFICADO**: gradle/lint/assemble/Android/UI/Room con DAOs reales (sin Android SDK).
Render real del parser en la app no probado en dispositivo.

**Hallazgos adicionales (descubrimiento continuo)**: este era el último hoyo de la familia
"día de mes suelto" (c.70 cubrió "el N del mes que viene", c.68 "el N" standalone). La
asimetría palabra-artículo ("el" vs "el día") era estructural: CUALQUIER patrón que
capture un día numérico del mes heredaba el mismo bug si no admitía "día"; el patrón de
fij es "añadir `(?:d[ií]a\s+)?` opcional al frente del grupo de día" de forma uniforme.
Queda abierto (P2 baja): "el día quince" (número ESCRITO, no dígito) → `dayOfMonthPattern`
solo captura `\d{1,2}`; forma menos común en móvil (la gente teclea dígitos), pero
registrar para futuro. Próxima auditoría debe salir del parser de días de mes y mirar otra
área de producto: captura ultrarrápida/inbox inteligente, recordatorios, rutinas
adaptables, What Now, o `RecurrenceEngine` edge cases (saltos DST, clamps fin-de-mes,
anclaje a día inexistente).

---

## Último trabajo — Ciclo 100: Parser — "cada fin de semana" (forma larga + "cada") caía a fecha única sin recurrencia (P2 hábito perdido, hallazgo c.99)

Bug **P2 (hábito de fin de semana perdido)**: `"cada fin de semana"` (forma larga con "cada")
no generaba recurrencia: `rec=NONE` + `due=próximo sábado` (fecha única). El usuario pedía un
hábito de fin de semana ("estudiar cada fin de semana") y la tarea aparecía UNA sola vez, el
recordatorio no repetía. Asimetría flagrante: `"cada finde"` (apócope) SÍ era hábito WEEKLY 6,7
(c.99 lo confirmó), pero la forma larga `"cada fin de semana"` no casaba
`weekendRecurrencePattern`. Preexistente (verificado en `5043282` sin cambios de c.99).

Causa raíz: `weekendPattern` (detección temprana de fecha → próximo sábado) se ejecuta ANTES
que `parseRecurrence` y consumía "cada fin de semana" completo (sin distinguir el determinante
"cada"). Al borrarse la frase antes del analizador de recurrencia, esta nunca la veía →
`weekendRecurrencePattern` solo casaba `fines de semana`/`findes?` (apócope), no la forma
larga con "cada". Doble asimetría: "cada finde"=hábito (OK) vs "cada fin de semana"=fecha (mal).

Solución (`NaturalTaskParser.kt`, cambio mínimo, reutiliza TODO el flujo existente):
1. `weekendPattern` (fecha) gana lookbehind negativo `(?<!cada\s)` y `(?<!los\s)` en la rama
   `fin|finales de semana` (igual que ya tenía en la rama `finde`). Así "cada fin de semana" y
   "los fines de semana" NO son consumidos por la rama de fecha → llegan intactos a
   `parseRecurrence`.
2. `weekendRecurrencePattern` (hábito) añade la alternancia `cada\s+fin\s+de\s+semana\b`.
   Reutiliza `detectWeekInterval()` ("cada dos semanas los fines de semana"), `WEEKLY` + días
   6,7, y la primera ocurrencia la resuelve la rama WEEKLY+days (próximo sábado).

Heurística honesta: el determinante de cadencia "cada"/"los" o el plural "fines" es señal real
de hábito; el singular sin "cada/los" ("este/el fin de semana", "finde" suelto con este/el)
sigue siendo fecha única. Sin IA, sin random.

Tests: +1 test permanente `cadaFinDeSemanaEsHabitoSemanalFinDeSemana`. Verificado sin
regresión: "este finde"/"fin de semana"/"este fin de semana" siguen NONE (fecha); "cada
finde"/"los findes"/"fines de semana"/"cada fin de semana" son WEEKLY 6,7.
`bash tools/run_domain_tests.sh` → **686 tests PASS** (685 + 1). Smoke
`bash tools/run_domain_checks.sh` → **25 OK**. NO VERIFICADO gradle/lint/assemble/Android/UI/Room
(sin Android SDK). Próxima auditoría: salir del parser de "fin de semana" (familia completa y
simétrica ahora) y mirar BACKLOG-16 ("cita en media hora y cuarto") u otra área de producto
(captura/What Now/recuperación de tareas vencidas).

---

## Último trabajo — Ciclo 99: Parser — "este finde" singular (apócope) se capturaba como recurrencia semanal en vez de fecha única (P1 over-recurrence)

(histórico; ver RUN_LOG c.99)

---

## Último trabajo — Ciclo 97: Parser — "cada dos semanas los lunes" (intervalo escrito + días) → WEEKLY+1 (doble frecuencia) en vez de WEEKLY+2 (P1 rutina mal programada)

Bug **P1 (rutina mal programada / pérdida de integridad de agenda)**: `"cada dos semanas
los lunes"` se agendaba al **DOBLE de frecuencia** (WEEKLY+1, cadencia semanal) en vez de
WEEKLY+2 (quincenal), Y dejaba "cada dos semanas" como residuo en el título. La persona
pedía una rutina quincenal y recibía una rutina **semanal**: los recordatorios disparaban
el doble de veces, la planificación de hábitos mostraba la cadencia errónea y la rutina
"permanente" se degeneraba. Asimetría flagrante: la forma con **dígitos** `"cada 2 semanas
los lunes"` ya funcionaba correctamente (c.54, `detectWeekInterval()` casa `\d{1,3}`), y el
intervalo escrito **sin días** `"cada dos semanas"` también (c.57, vía `intervalPattern`),
pero la **combinación escrita + lista de días** caía en el bug porque:
- `parseRecurrence` procesa la **rama de lista de días** (que admite "cada N semanas los
  lunes/de lunes a viernes") **antes** que `intervalPattern` (intervalo sin días).
- El helper de esa rama, `detectWeekInterval()`, solo reconocía `\d{1,3}` para "cada N
  semanas", NO números escritos ("dos"/"tres") → devolvía `null` → interval por defecto
  **1** (cadencia semanal, el doble de frecuente) → la frase "cada dos semanas" NO se
  consumía como intervalo y quedaba como residuo en el título.
Así la misma intención expresada con palabra ("dos") en vez de dígito ("2") producía una
cadencia **completamente distinta** — un P1 de integridad de datos: el plan recurrente
que el usuario creía haber configurado NO era el que se almacenaba.

**Solución (mínima, `NaturalTaskParser.kt`, sin nueva pantalla/botón — "menos interfaz,
más potencia")**: `detectWeekInterval()` ahora admite números escritos: la regex pasa de
`\bcada\s+(\d{1,3})\s*semanas?\b` a
`\bcada\s+(\d{1,3}|$writtenNumberGroup)\s*semanas?\b`, y `parseWrittenNumber` (helper
existente, reutilizado) resuelve la palabra a entero (coerceIn 1..366). Para que
`detectWeekInterval()` —que aparece **antes** que `intervalPattern` en el cuerpo de
`parseRecurrence`— pueda referenciar `writtenNumberGroup`, este `val` se **trasladó** al
**inicio** de `parseRecurrence` (antes compartido solo por `intervalPattern`); se eliminó
la declaración duplicada de su antigua ubicación. Así `"cada dos semanas los lunes"` →
WEEKLY+2 (igual que `"cada 2 semanas los lunes"`) y la frase completa se consume → título
limpio ("Gym cada dos semanas los lunes" → "Gym"). Simétrico al comportamiento de dígitos
y al intervalo escrito sin días. Lógica local honesta (mapeo de token, sin random ni
modelo simulado). Retrocompatible (sin cambios de firma pública).

**Tests**: +4 en `NaturalTaskParserTest.kt` (TDD RED antes del fix): `writtenBiweeklyIntervalWithWeekdayRangeCombinesIntervalAndDays` ("Gym cada dos semanas de lunes a viernes" → WEEKLY+2 días[1..5] título "Gym"), `writtenBiweeklyIntervalWithDayListCombinesIntervalAndDays` ("Gym cada dos semanas los lunes y viernes" → WEEKLY+2 días[1,5]), `writtenTriweeklyIntervalWithDayListCombinesIntervalAndDays` ("Clase cada tres semanas los martes y jueves" → WEEKLY+3 días[2,4]), y `writtenBiweeklyIntervalWithSingleDayCombinesIntervalAndDay` ("Estudio cada dos semanas los lunes" → WEEKLY+2 días[1]). Antes del fix los 4 fallaban con `rec=WEEKLY+1` y residuo " cada dos semanas"/" cada tres semanas" en el título. `bash tools/run_domain_tests.sh` → **675 PASS** (28 clases, 671 previos + 4); `bash tools/run_domain_checks.sh` → smoke 25 OK. Probe JVM amplio confirmó GREEN sin regresión: `"cada dos semanas los lunes"`→WEEKLY+2 días[1], `"cada dos semanas los lunes y viernes"`→WEEKLY+2 días[1,5], `"cada tres semanas los lunes"`→WEEKLY+3 días[1], y `"cada 2 semanas los lunes"` (dígitos) sigue →WEEKLY+2 (no-regresión). Con el día base miércoles 2026-07-29, el primer lunes de la cadencia queda 2026-08-03 (correcto, próxima ocurrencia del día objetivo).
...
## Último trabajo — Ciclo 96: Parser — "a la una" (hora 1, femenino singular) no se parseaba → cita sin vencimiento + título sucio (P1 captura/agenda)

Bug **P1 (captura perdida / cita olvidada)**: la hora **1** en español se dice
`"a la una"` (femenino singular, conector "a la"), no `"a las 1"`. El patrón
`a las N` de `timePatterns` excluía deliberadamente `un/una/uno` de
`WRITTEN_HOUR_ALT` (para no confundirlo con el artículo/det.), así que
`"reunión a la una"` **no casaba ningún patrón de hora** → `dueAt=null` Y la
frase "a la una" quedaba como residuo en el título. La cita nacía sin
vencimiento → invisible en "What Now"/planificador, recordatorio imposible.
Además `"a la una del mediodía"` (forma cotidiana de 1pm) caía a la canónica
NOON (12:00) en vez de 13:00: la rama `mv.contains("mediodía")` de
`explicitTimeData` cortocircuitaba ANTES de procesar la hora capturada.

- **Síntoma (probe JVM, now=2026-07-29 12:00, zone=America/Guatemala)**:
  `reunión a la una` → `dueAt=null title='reunión a la una'` (BUG, debería 01:00);
  `reunión a la una de la tarde` → `dueAt=null` (BUG, debería 13:00);
  `reunión a la una y media` → `dueAt=null` (BUG, debería 01:30);
  `reunión a la una del mediodía` → `dueAt=12:00` (BUG, debería 13:00).
- **Causa raíz**: (1) ausencia de patrón para la forma femenino-singular
  "a la una" (la regex `a las N` exige plural "las" + dígito/hora escrita ≠ 1);
  (2) la rama NOON/MIDNIGHT de `explicitTimeData` disparaba por presencia de la
  subcadena "mediodía" en el valor coincidido, **independientemente de que se
  hubiera capturado una hora** → "a la una del mediodía" se tragaba como NOON.
- **Solución mínima (sin nueva pantalla/botón — "menos interfaz, más potencia")**:
  - Nuevo patrón `a la una` al **frente** de `timePatterns` (para ganarle al
    `a las N` que exige "las"), con el **mismo layout de grupos** que ese
    (1=hora, 2=:MM, 3=y media|cuarto, 4=meridiem, 5=horas). `parseHour`
    ya mapea "una"→1 (verificado). Así `explicitTimeData` lo procesa en la
    rama genérica **sin ramificación nueva**.
  - El meridiem `del mediodía` se añade a la alternación de ambos patrones
    (`a la una` y `a las N`), y `isPm` reconoce `delmediodía`/`delmediodia`
    (PM: hora 1 → 13). `isAm`/`isPm` existentes intactos para las demás formas.
  - La rama NOON/MIDNIGHT de `explicitTimeData` se **protege con guarda de
    grupos vacíos**: solo aplica a frases puras "al mediodía"/"a la medianoche"
    (patrón sin grupo de hora), i.e. cuando los grupos 1 (hora) y 2 (minutos)
    están en blanco. Así "a la una del mediodía" (grupo 1 = "una") cae a la rama
    genérica y resuelve 13:00, no NOON.
  - Lógica local honesta (mapeo de token + offset PM), sin random ni modelo
    simulado. Retrocompatible (sin cambios de firma pública).
- **Tests**: +6 en `NaturalTaskParserTest.kt`:
  `aLaUnaParsesOneOclockAndCleanTitle` (01:00 + título "Reunión"),
  `aLaUnaYMediaParsesHalfPastOne` (01:30),
  `aLaUnaYCuartoParsesQuarterPastOne` (01:15),
  `aLaUnaDeLaTardeParsesOnePm` (13:00),
  `aLaUnaDelMediodiaParsesOnePmNotNoon` (13:00, **NO** 12:00),
  `aLaUnaColonMinutesParsesCorrectly` (01:30 vía `:MM`).
  `bash tools/run_domain_tests.sh` → **671 PASS** (28 clases, 665 previos + 6);
  `bash tools/run_domain_checks.sh` → smoke 25 OK. Probe JVM amplio (7 casos)
  confirmó sin regresión: `"a la una"`=01:00, `"a la una de la tarde"`=13:00,
  `"a la una del mediodía"`=13:00 (NO 12:00), `"a la una y media"`=01:30,
  `"a la una y cuarto"`=01:15, `"a la una:30"`=01:30, y `"al mediodía"` puro
  sigue = NOON (12:00) intacto.
- **NO VERIFICADO**: gradle/lint/assemble/Android/UI/Room con DAOs reales (sin
  Android SDK). Render real del parser en la app no probado en dispositivo.
- **Hallazgos adicionales (descubrimiento continuo)**: la asimetría
  "a las N" (plural) vs "a la una" (singular) es la última trampa conocida del
  género femenino de la hora 1. El corto-circuito NOON por subcadena era un
  P1 latente que afectaba a cualquier hora con meridiem "del mediodía" (no solo
  la 1): por construcción ahora es seguro para toda hora. Próxima auditoría
  debe salir del parser de horas puntuales y mirar otra área (captura rápida,
  recordatorios, rutinas).
- **Archivos modificados**: `app/src/main/java/com/ordia/app/domain/NaturalTaskParser.kt`,
  `app/src/test/java/com/ordia/app/domain/NaturalTaskParserTest.kt`,
  `AI_AUTONOMY/{BACKLOG,CURRENT_STATE,RUN_LOG}.md`.
- **Commits**: 1 (`fix(parser): "a la una" (hora 1) + "del mediodía" NOON guard (P1)`).
- **Estado**: FIXED → VERIFIED (dominio JVM). 671 domain tests PASS.

## Último trabajo — Ciclo 94: Parser — "en media hora"/"en un cuarto de hora" se parseaban como DURACIÓN (sin vencimiento) en vez de punto en el tiempo (P1 captura perdida)

Bug **P1 (captura perdida / recordatorio imposible)**: `"en media hora"` /
`"en un cuarto de hora"` / `"dentro de media hora"` / `"de aquí a media hora"` se
parseaban como **duración** (sin vencimiento) en vez de **punto en el tiempo**.
`relativePattern` solo aceptaba números escritos enteros (un…treinta) o dígitos, NO las
fracciones cotidianas "media hora" (30 min) / "(un) cuarto de hora" (15 min). Estas caían
a `fractionalDurationPattern` → `dueAt=null`, `durationMinutes=30/15` y el prefijo
"en"/"dentro de" quedaba como residuo en el título ("llamar en media hora" → título
"llamar en"). La tarea quedaba SIN vencimiento → invisible en "What Now"/planificador,
recordatorio imposible de programar.

- **Síntoma (probe JVM, now=2026-07-29 12:00)**: `llamar en media hora` →
  `due=null dur=30 title='llamar en'` (BUG, debería +30min); `reunión dentro de media
  hora` → idem BUG; `cita de aquí a media hora` → idem BUG; `en una hora` → +60min OK
  (asimetría: el entero escrito funcionaba, la fracción no); `reunión media hora` (sin
  prefijo) → `dur=30 due=null` (duración correcta, NO debe romperse).
...
- **Solución mínima**: nuevo `fractionalRelativePattern` (prefijo
  ...
  incluido) → título limpio ("llamar en media hora" → "llamar"). Incluido en
  `effectiveRelativeDueAt` (misma prioridad que `relativeDueAt`) y `relativeIsDays=false`
  (es sub-hora: la hora explícita no la sobreescribe).
- **No-regresión (crítica)**: el prefijo es **obligatorio** → "reunión media hora" (sin
  prefijo) sigue siendo `durationMinutes=30` (duración real, no fecha); "media hora antes"
  (recordatorio) lo captura `reminderPatterns` antes y no choca; "en una hora" (entero
  escrito) sigue funcionando sin conflicto.
...
- **Tests (TDD)**: +7 (`enMediaHoraEsFechaRelativa`, `enUnCuartoDeHoraEsFechaRelativa`,
  ...
  `deAquiAMediaHoraEsFechaRelativa`, `mediaHoraSinPrefijoSigueSiendoDuracion`,
  más la verificación de no-regresión cubre `durationMinutes`). Probe JVM confirmó RED antes
  del fix (`due=null`) y GREEN tras (`due=now+30min`). Comando:
  `bash tools/run_domain_tests.sh` → **655 tests PASS** (27 clases — 649 base (c.93) + 7 nuevos).
  Smoke: `bash tools/run_domain_checks.sh` → **25 assertions OK**. Sin regresión
  (`en una hora`, `en treinta minutos`, `media hora` duración, `media hora antes`
  recordatorio siguen OK).
- **NO VERIFICADO**: gradle/lint/assemble/Android/UI/Room con DAOs reales (sin Android SDK).
- **Archivos modificados**: `app/src/main/java/com/ordia/app/domain/NaturalTaskParser.kt`,
  `app/src/test/java/com/ordia/app/domain/NaturalTaskParserTest.kt`,
  `AI_AUTONOMY/{BACKLOG,CURRENT_STATE,RUN_LOG}.md`.
...
## Último trabajo — Ciclo 91: Parser — "último día del mes" no se parseaba (P1 pérdida de datos)

---

## Último trabajo — Ciclo 93: Parser — recurrencias en forma ADJETIVA ("pago mensual"/"reunión semanal"/"renta anual"/"reporte quincenal"/"factura bimestral"/"impuesto trimestral"/"cierre semestral") caían a NONE (P1 compromiso periódico olvidado) (fix(parser): "en media hora"/"en un cuarto de hora" ya son fecha relativa (P1))

Bug **P1 (compromiso periódico olvidado / pérdida silenciosa de recurrencia)** en
`NaturalTaskParser.parseRecurrence`. Las formas ADJETIVAS cotidianas de recurrencia
(`mensual`, `semanal`, `anual`, `quincenal`, `bimestral`, `trimestral`, `semestral`) NO se
reconocían: solo se captaba el adverbio `-mente` (`mensualmente`, `semanalmente`, …) y, para
el caso quincenal, las frases `cada quincena` / `quincenalmente` / `todas las quincenas`.

- **Síntoma**: `"pago mensual"` → `rec=NONE interval=1 due=null`; `"renta anual"` → NONE;
  `"factura bimestral"` → NONE; `"cierre semestral"` → NONE; `"reporte quincenal"` (adjetivo,
  sin "-mente") → NONE. La tarea nacía **sin cadencia**: no aparecía en el planificador de
  hábitos, el recordatorio disparaba una sola vez y nunca más → compromiso periódico olvidado.
  La fecha explícita sí se conservaba (`"pago mensual el 10"` → dueAt=día 10, pero `rec=NONE`),
  así que el bug era aún más sutil: la persona veía UN vencimiento y creía que la recurrencia
  estaba puesta. Solo se reconocían vía numeral (`"cada 2/3/6 meses"`) o adverbio (`-mente`).
- **Causa raíz**: en `parseRecurrence` el `fixedPatterns` (línea ~1590) solo listaba adverbios
  `\bsemanalmente\b`, `\bmensualmente\b`, `\banualmente\b`; el bloque quincenal autónomo
  (línea ~1564) solo casaba `quincenalmente` (no el adjetivo `quincenal`); y los plurimensuales
  `bimestral`/`trimestral`/`semestral` no tenían ninguna rama (solo existía el numeral
  `cada N meses`). Así el adjetivo caía a `base = RecurrenceResult(NONE, …)`.
- **Solución mínima (sin nueva pantalla/botón, sin enum ni migración)**:
  1. Bloque quincenal: `quincenalmente` → `quincenal(?:mente)?` (captura también el adjetivo
     `quincenal`); sigue mapeando a `WEEKLY interval=2` (cada 2 semanas ≈ quincena).
  2. Nuevo bloque plurimensual (antes de `fixedPatterns`, porque éste solo admite
     `interval=1`): `bimestral(?:mente)?`→`MONTHLY+2`, `trimestral(?:mente)?`→`MONTHLY+3`,
     `semestral(?:mente)?`→`MONTHLY+6`. Reutiliza `RecurrenceEngine` que ya avanza
     `plusMonths(interval)`.
  3. `fixedPatterns`: añadido `|\bsemanal\b` (WEEKLY), `|\bmensual\b` (MONTHLY),
     `|\banual\b` (YEARLY) — los adjetivos de intervalo 1. Los límites `\b` evitan colisionar
     con el adverbio (`\bmensual\b` no casa dentro de "mensualmente") ni con otras palabras
     (`\banual\b` no casa dentro de "manual").
- **No-regresión**: los adverbios `-mente` siguen funcionando (probe: `mensualmente`/
  `quincenalmente`/`bimestralmente` OK); `"reunión semanal los lunes"` sigue cayendo en la rama
  de lista de días (devuelve WEEKLY+días antes de llegar a `fixedPatterns`);
  `"cada 2/3/6 meses"` (numeral) inalterado; la fecha explícita sigue teniendo prioridad
  (`"pago mensual el 10"` → MONTHLY+1 anclado al 10).
- **Tests (TDD)**: +8 (`adjetivoMensualParsesMonthlyRecurrence`,
  ...
  `adjetivoMensualRespetaFechaExplicita`). Probe JVM confirmó RED antes del fix
  (`rec=NONE interval=1 due=null` para los 7 adjetivos) y GREEN tras. **649 domain tests
  PASS** (`bash tools/run_domain_tests.sh`, 27 clases — subida desde 641); **smoke 25 OK**
  (`bash tools/run_domain_checks.sh`). Sin regresión.
- **NO VERIFICADO**: gradle/lint/assemble/Android/UI/Room con DAOs reales (sin Android SDK).
- **Filosofía**: sin nueva pantalla/botón. Recupera compromisos periódicos comunes (pagos,
  rentas, impuestos, cierres) que el parser descartaba por usar la forma adjetiva en vez del
  numeral/adverbio. Reutiliza la infraestructura de recurrencia existente (MONTHLY+interval,
  RecurrenceEngine.plusMonths) — potencia sin nueva complejidad visible.
- **Rebase**: este ciclo se basó en `d3734ed` (c.90) pero el remoto avanzó con c.91
  ("último día del mes") y c.92 ("cierre de mes") mientras tanto. Rebase limpio: el código
  (.kt) auto-mergeó sin conflictos (áreas distintas del parser); solo los archivos de memoria
  (CURRENT_STATE/RUN_LOG) requirieron reconciliación manual preservando el trabajo de ambos
  runs. Mi commit original fue `1f8cce9`; renumerado a c.93 tras el rebase.

---

## Último trabajo — Ciclo 92: Parser — "cierre de mes"/"cierre del mes" no se parseaba (P1 pérdida de datos)

Bug **P1 (pérdida de datos silenciosa)**: `"cierre de mes"` / `"cierre del mes"` — sinónimo
cotidiano de "fin de mes" en contexto financiero (alquileres, facturas, nómina, contabilidad)
— no se parseaba. `endOfMonthPattern` solo reconocía `fin`/`finales`/`último día` (c.91),
así `"pago cierre de mes"` caía en `dueAt=null` y la frase quedaba como título → el
vencimiento se olvidaba (sin recordatorio, invisible en planificador/What Now). Pagos y
rentas dichos así se perdían, igual que `último día del mes` (c.91). Continúa el
descubrimiento continuo de sinónimos de "fin de mes" apuntado en "próxima prioridad" del c.91.

- **Síntoma (probe JVM, now=2026-07-15)**: `pago cierre de mes` → `due=null` (BUG, debería
  07-31). `renta cierre del mes` → `due=null` (BUG). `pago vence fin de mes` → 07-31 (OK,
  ya casa vía `fin de mes`). `pago vence el mes` → null (ambiguo, dejado fuera: "vence el
  mes" a secas no implica fin de mes).
- **Causa raíz**: `endOfMonthPattern` no incluía `cierre` como alternativa. Por eso
  "cierre de mes" no casaba y la frase quedaba como título → dueAt=null.
  `monthBaseForBoundary` tampoco reconocía `cierre` en su rama "end" (aunque al no casar el
  regex nunca llegaba ahí).
- **Solución mínima**:
  - `endOfMonthPattern`: añadir alternativa `cierre` →
  ...
  - `monthBaseForBoundary`: rama "end" ahora incluye `t.contains("cierre")`. El modificador
    "que viene"/"próxim" ya se resuelve ANTES (línea `isNext`), así "cierre del mes que
    viene" ancla al mes siguiente sin doble-desplazamiento.
...
- **Tests (TDD)**: +4 (`cierreDeMesAnclaFinDeMesActual`,
  ...
  `cierreDeMesRespetaHoraExplicita`) en nuevo `NaturalTaskParserCierreTest.kt`. Probe JVM
  confirmó RED antes del fix (`due=null` en ambos) y GREEN tras (`due=07-31` actual /
  `08-31` "que viene"; hora explícita `a las 18` respeta fecha fin de mes). Comando:
  `bash tools/run_domain_tests.sh` → **641 tests PASS** (base c.91=637 + 4 nuevos; ambos
  fixes coexisten). Smoke: `bash tools/run_domain_checks.sh` → **25 OK**. Sin regresión
...
## (Histórico) Ciclo 89: Parser — bare-plural "sabados" sin tilde perdía la rutina (P1)

Bug **P1 (pérdida de datos silenciosa)** de asimetría de acentos en el parser de
recurrencias. Simétrico al c.88 ("próximo" sin tilde) y al c.41 (plurales sábados/domingos
en la regex). El c.41 añadió `s[aá]bados?|domingos?` al `dayListPattern` (la regex casa
ambas formas), pero el guard `barePluralSingle` que decide si un único día bare es
recurrencia (no fecha) solo comprobaba `g.contains("sábados")` **con tilde**.

- **Síntoma**: `"fútbol sabados"` (sin tilde — la escritura habitual en captura móvil) →
  `recurrence=NONE`, `recurrenceDays=''`, `title='fútbol sabados'`. El día quedaba como
  residuo en el título y la rutina semanal se olvidaba en silencio: sin recurrencia, sin
  recordatorio recurrente, invisible en el planificador de hábitos. La forma con tilde
  `"sábados"` ya funcionaba (WEEKLY 6). La lista de 2+ días `"gym sabados y domingos"`
  también funcionaba (por la rama `days.size >= 2`, que no pasa por el guard). Solo fallaba
  el caso más común de hábito: un único día bare plural de sábado, sin tilde.
- **Causa raíz**: `NaturalTaskParser` línea ~1456 `barePluralSingle = ... g.contains("sábados")
  || g.contains("domingos")` — solo la forma acentuada de sábado. La regex de lista ya casa
  `s[aá]bados?`, así que "sabados" llegaba al guard pero lo pasaba como no-plural → no se
  reconocía como recurrencia de un solo día bare → caía fuera del `if` y la frase no se
  consumía (quedaba en el título, `recurrence=NONE`). Descuido del c.41, no una decisión.
- **Solución mínima**: `g.contains("sábados") || g.contains("sabados") || g.contains("domingos")`
  — alineado con la regex `s[aá]bados?` del c.41. Sin tocar el regex ni el flujo de
  `weekdaySameDayCandidate`. El día suelto no plural `"reunión martes"` sigue siendo fecha
  ambigua (no-regresión, `bareSingleNonPluralDayIsNotRecurrence`).
- **Tests (TDD)**: +2 (`parsesBarePluralSingleDayRecurrenceUnaccented`,
  `parsesBareDayListUnaccentedSabado`). Probe JVM confirmó RED antes del fix
  (`freq=NONE days=''` para "fútbol sabados") y GREEN tras (`freq=WEEKLY days='6'`).
  **622 domain tests PASS** (`bash tools/run_domain_tests.sh`, 27 clases — subida desde
  620); **smoke 25 OK** (`bash tools/run_domain_checks.sh`). Sin regresión.
- **Auditoría de acentos completada (TASK-3)**: los 7 chequeos `contains`/`==` acento-sensibles
  del parser están balanceados: `mediodía`/`mediodia` (línea 977) y `próxim`/`proxim`
  (líneas 925 y 1614) ya cubren ambas formas; el meridiem `delamañana`/`delamanaana` (línea 1011)
  cubre la forma sin tilde habitual (probe: `"9 de la manana"` → parses OK con dueAt); `sabados`
  fixed (línea 1457). No quedan asimetrías de acentos activas en el parser.
- **NO VERIFICADO**: gradle/lint/assemble/Android/UI/Room con DAOs reales (sin Android SDK).
- **Filosofía**: sin nueva pantalla/botón. Corrige una decisión errónea del parser y elimina
  un descuido de normalización de acentos → el hábito semanal más típico se recuerda sin
  exigirle al usuario tildes. Protege datos (la rutina no se olvida).

## Último trabajo — Ciclo 88: Parser — "próximo" sin tilde ("proximo viernes") dicho hoy = ese día → HOY en vez de +7 (P1 integridad de agenda)

Bug **P1 (pérdida de cita/recordatorio erróneo)** de asimetría de acentos en el parser de
días de semana. Simétrico al c.72 (que fijó "jueves que viene"/"próximo" dicho en el propio
día objetivo), pero el c.72 solo detectaba la forma **acentuada** `"próxim"`. La escritura
sin tilde `"proximo"` —habitual en captura rápida móvil— caía en HOY.

- **Síntoma**: en viernes, `"reunión el proximo viernes"` (sin tilde) agendaba HOY (viernes
  actual) en vez de la próxima semana (+7). Idéntico con sufijo `"el viernes proximo"`. La
  forma acentuada `"próximo"` ya funcionaba. El recordatorio se disparaba ~7 días antes de
  la cita real → la persona perdía o duplicaba el evento. Dependencia sutil de hora del día:
  solo se manifestaba antes de la hora canónica 09:00 (tras ella, el rollover anti-pasado lo
  "arreglaba" por accidente).
- **Causa raíz**: `NaturalTaskParser` línea 919 `nextExplicit = mv.contains("que viene") ||
  mv.contains("próxim")` — solo la forma acentuada. La línea 1608 (`monthBaseForBoundary`)
  ya aceptaba ambas (`"próxim" || "proxim"`), así que la asimetría era un descuido, no una
  decisión. Con `nextExplicit=false`, se usaba `nextWeekdayOrSame` (devuelve hoy) en vez de
  `nextWeekday` estricto (+7).
- **Solución mínima**: `nextExplicit = mv.contains("que viene") || mv.contains("próxim") ||
  mv.contains("proxim")` — alineado con la línea 1608. Sin tocar el regex (ya soporta ambas
  formas vía `pr[oó]ximo`) ni el flujo de `weekdaySameDayCandidate`. El día suelto "el viernes
  a las 18" dicho en viernes sigue pudiendo ser hoy (no-regresión, `nextExplicit=false`).
- **Tests (TDD)**: +3 (`proximoSinTildeViernesHoyFuerzaProximaSemana`,
  ...
  con `now`=viernes 8:00 (antes de la 09:00 canónica, donde el bug se manifestaba). Confirmado
  RED antes del fix (2 failures: `expected:<2026-08-07> but was:<2026-07-31>`) y GREEN tras.
  **620 domain tests PASS** (`bash tools/run_domain_tests.sh`); **smoke 25 OK**
  (`bash tools/run_domain_checks.sh`). Sin regresión.
- **NO VERIFICADO**: gradle/lint/assemble/Android/UI/Room con DAOs reales (sin Android SDK).
- **Filosofía**: sin nueva pantalla/botón. Corrige una decisión errónea del parser y elimina
  un descuido de normalización de acentos → la cita va al día correcto sin exigirle al usuario
  tildes. Protege datos (la cita no se agenda una semana antes).

## Último trabajo — Ciclo 86: Búsqueda + Inteligencia — fix "esta semana" en domingo + consolidar `timeRank` DRY

Dos unidades atómicas en este run, ambas sobre el dominio (sin UI nueva, "menos interfaz,
más potencia").

**1) Fix P1 — `SearchEngine.taskMatchesDateScope(ThisWeek)` arrastraba la semana siguiente en
domingo.** `daysToSunday = 7 - (today.dayOfWeek.value % 7)`. En domingo (`value=7`), `7 % 7 = 0`
...
**Causa raíz**: el operador `%` se aplicaba después de la resta en vez de a la resta completa.

**Fix (mínimo)**: `(7 - today.dayOfWeek.value) % 7`. Domingo da 0 (la semana termina hoy); lunes
da 6 (domingo, sin cambio). La atrasada sigue excluida del rango semanal (tiene su propio filtro
OVERDUE). Test `estaSemana_onSundayEndsTodayNotNextWeek`: en domingo 2026-08-16 el lunes siguiente
(id 2) ya no aparece; la atrasada (id 3) sigue fuera. Confirmado FAIL antes del fix, PASS después.

**2) Refactor P2 — consolidar `timeRank` en `TaskRules` como fuente única.** `WhatNowEngine`
tenía una **copia privada de `timeRank` idéntica** a la de `TaskRules`, el mismo patrón de
duplicación que causó divergencia silenciosa con `priorityScore` (fix c.53). Cualquier edición
futura en uno los habría desincronizado: el ranking de What Now (tarjeta/asistente) y el del widget
(`TaskRules.nextBestTask`) deben coincidir pero vivían en dos sitios. Probe divergencia: MATCH en
5 instantes (sin bug activo), pero deuda de diseño latente.

**Solución**: `TaskRules.timeRank` pasa de private a public (fuente única de verdad);
`WhatNowEngine.ordered` delega en `TaskRules.timeRank` y elimina su copia privada + el `val today`
muerto. Test guardia `whatNowAndWidgetAgreeOnBestTaskAcrossTime`: conjunto diverso (vencida, en
curso, urgente, inbox, programada) evaluado en 5 instantes distintos; falla si What Now y el
widget vuelven a divergir.

**Tests**: `bash tools/run_domain_tests.sh` → **617 tests PASS** (612 base c.86 + 5 nuevos);
`bash tools/run_domain_checks.sh` → **smoke 25 OK**.
**NO VERIFICADO**: gradle/lint/assemble/Android/UI/Room con DAOs reales (sin Android SDK).
...
## Último trabajo — Ciclo 87: Búsqueda — "semana que viene"/"próxima semana"

Feat P2 (recuperación de información) en `SearchEngine`. La búsqueda por intención de fecha
añadida en c.81 cubría hoy/mañana/esta semana/atrasadas, pero **NO distinguía "esta semana" de
"semana que viene"/"próxima semana"**: como `WEEK_TOKENS = {semana}` no distinguía modificador,
"semana que viene" caía a `THIS_WEEK` → el usuario que busca qué vence la semana próxima veía la
semana actual. La distinción planificar-próxima-semana se perdía, rompiendo la promesa de
"búsqueda universal".

- **Causa raíz**: `detectDateScope` no diferenciaba "semana" con/sin modificador de proximidad.
- **Solución mínima**: nuevo `DateScope.NEXT_WEEK`; `NEXT_WEEK_TOKENS = {proxima, proximas,
  viene}`; rama `hasWeek && hasNext → NEXT_WEEK` antes de `hasWeek → THIS_WEEK`. Rango
  `startNextWeek = today + daysToSunday + 1` (lunes próximo), `endNextWeek = startNextWeek + 6`
  (domingo próximo), reusando la fórmula `(7 - dayOfWeek) % 7` del fix de domingo del c.86.
  `dateScopeTokens` incluye `NEXT_WEEK_TOKENS` para no exigir "viene"/"proxima" en el contenido.
- **No-regresión**: "esta semana" y "semana" sola siguen siendo `THIS_WEEK`.
- **Tests**: +5 (`semanaQueViene_returnsOnlyNextWeekTasks`, `proximaSemana_returnsOnlyNextWeekTasks`,
  `semanaQueViene_excludesThisWeekTasks`, `semanaSolaSigueSiendoEstaSemana`,
  `semanaQueViene_onSundayStartsNextMonday`). **617 domain tests PASS**; **smoke 25 OK**.
- **NO VERIFICADO**: gradle/lint/assemble/Android/UI/Room con DAOs reales (sin Android SDK).

## Último trabajo — Ciclo 85: Parser — duraciones con número escrito ("dos horas"/"treinta minutos"/"un par de horas")

Fix P1 (capacidad de tarea/inteligencia) en `NaturalTaskParser`. La **duración** era la única
superficie de cantidad que NO aceptaba números escritos: los recordatorios (c.40) y las fechas
relativas (c.35/c.57) SÍ los aceptaban, pero `durationPatterns` solo casaba `\d{1,3}` (dígitos),
y la duración fraccionaria solo cubría "media hora"/"cuarto de hora". Así "estudiar dos horas",
"llamada de treinta minutos", "reunión de una hora", "un par de horas" caían a
`durationMinutes=null` → el planificador las trataba como `TaskRules.MIN_PLAN_MINUTES` (10 min),
el `SummaryEngine.DayLoad` subestimaba la carga real del día y `WhatNowEngine` perdía noción del
progreso real. Deficiencia de **inteligencia silenciosa** (no crasheaba, pero degradaba las
decisiones automáticas centrales), asimetría con el resto del parser.

- **Causa raíz**: ausencia de un patrón de duración con cantidad escrita. `parseWrittenNumber`
  y `writtenAmountPattern` ya existían y se reusan; solo faltaba conectarlos al cómputo de
  `durationMinutes`.
- **Solución mínima**: nuevo `writtenDurationPattern = \b($writtenAmountPattern)\s*(minutos?|min|horas?|hora)\b`
  (solo minutos/horas: la duración se acota a ≤24 h, así que "dos días" no es una duración
  significativa y se deja fuera). Se procesa con los **mismos guards** que la duración numérica
  (`timePhrasePreceding` — para no robar "a las nueve horas" como duración— y `en$` — para no
  robar "en dos horas" como duración, ya consumida por la fecha relativa antes). Recordatorios
  y fechas relativas se procesan **antes** y consumen sus frases ("dos horas antes", "en dos
  horas"), así que la duración escrita solo casa con la forma **bare**. El `when` elige la
  ocurrencia más a la izquierda entre {dígitos, escrita, fraccionaria}; el borrado del conector
  `de/durante/por` es simétrico al de la duración numérica ("reunión de dos horas" → "Reunión").
- **Tests**: +9 (`dosHorasEscritasEsDuracionDe120Min`, `unaHoraEscritasEsDuracionDe60Min`,
  ...
  **610 domain tests PASS** (`bash tools/run_domain_tests.sh`); **smoke 25 OK**
  (`bash tools/run_domain_checks.sh`). Sin regresión: las 8 pruebas existentes de duración
  fraccionaria/digit/compacta y las de recordatorio pasan intactas.
- **NO VERIFICADO**: gradle/lint/assemble/Android/UI/Room con DAOs reales (sin Android SDK).
- **Filosofía**: sin nueva pantalla/botón. Reusa la fuente única `writtenAmountPattern`/`parseWrittenNumber`
  y los guards existentes. Cierra la asimetría de cantidades escritas (duración era la última
  superficie sin soporte) → mejor decisión automática (plan/carga/What Now) sin más interfaz.
