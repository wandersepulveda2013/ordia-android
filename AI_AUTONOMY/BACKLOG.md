...
| P0 | Updater | Actualizador in-app nativo basado en manifiesto (check sin bloquear arranque, progreso, verificación SHA-256/package/versionCode/firma, PackageInstaller + confirmación final, Ajustes → Actualizaciones, badge, feed desacoplado del agente) | sesión 006: `UpdateManifestParserTest` 11/11 + `UpdateSecurityRulesTest` 9/9 + 6 variantes compilan + lint 0 errores; CI publica `update-manifest-<flavor>.json` + `Ordia-3.0-<flavor>-signed.apk` | VERIFIED — código/test/CI; falta verificación física en dispositivo ADB (bloqueada por hardware) |
