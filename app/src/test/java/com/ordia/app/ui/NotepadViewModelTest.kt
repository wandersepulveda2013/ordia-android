...
class NotepadViewModelTest {
    ...
    @Test
    fun save_nonEmptyNewNote_saves() = runTest {
    ...
    fun save_emptyExistingNote_deletesIt() = runTest {
        val id = dao.insert(NoteEntity(title = "Old", content = "Old", createdAt = 1, updatedAt = 1))
        viewModel.save("", "", id)
        assertTrue(dao.notes.isEmpty())
    }

    @Test
    fun save_emptyExistingNoteWithSpaces_deletesIt() = runTest {
        val id = dao.insert(NoteEntity(title = "Old", content = "Old", createdAt = 1, updatedAt = 1))
        viewModel.save("   ", " \n", id)
        assertTrue(dao.notes.isEmpty())
    }
    ...
    fun save_nonEmptyExistingNote_updatesIt() = runTest {
        val id = dao.insert(NoteEntity(title = "Old", content = "Old", createdAt = 1, updatedAt = 1))
        viewModel.save("New", "New", id)
        assertEquals(1, dao.notes.size)
        assertEquals("New", dao.notes.first().title)
    }
}
