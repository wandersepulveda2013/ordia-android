...
class NotepadViewModelTest {
    ...
    @Test
    fun save_emptyNote_doesNotInsert() = runTest(testDispatcher) {
        val dao = FakeDao()
    ...
    @Test
    fun save_emptyTitleAndContentWithSpaces_doesNotInsert() = runTest(testDispatcher) {
        val dao = FakeDao()
        val repo = NoteRepository(dao)
        val viewModel = NotepadViewModel(repo)
    ...
    @Test
    fun save_nonEmptyNote_inserts() = runTest(testDispatcher) {
        val dao = FakeDao()
        val repo = NoteRepository(dao)
        val viewModel = NotepadViewModel(repo)
    ...
    @Test
    fun save_updateToEmpty_deletesNote() = runTest(testDispatcher) {
        val dao = FakeDao(mutableListOf(NoteEntity(id = 1, title = "A", content = "B", createdAt = 1, updatedAt = 1, pinned = false)))
        val repo = NoteRepository(dao)
        val viewModel = NotepadViewModel(repo)
    ...
    @Test
    fun save_updateToNonEmpty_updatesNote() = runTest(testDispatcher) {
        val dao = FakeDao(mutableListOf(NoteEntity(id = 1, title = "A", content = "B", createdAt = 1, updatedAt = 1, pinned = false)))
        val repo = NoteRepository(dao)
        val viewModel = NotepadViewModel(repo)
