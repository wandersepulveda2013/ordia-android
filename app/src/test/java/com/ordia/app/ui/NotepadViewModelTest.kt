...
class NotepadViewModelTest {
    ...
    @Test
    fun save_emptyNewNote_isNotSaved() = runTest {
        val dao = FakeDao()
    ...
    fun save_nonEmptyNewNote_isSaved() = runTest {
        ...
        assertEquals("A", dao.notes[0].title)
    }
    ...
    @Test
    fun save_nonEmptyExistingNote_isUpdated() = runTest {
        val dao = FakeDao(mutableListOf(NoteEntity(id = 1, title = "A", content = "B", createdAt = 0, updatedAt = 0)))
        val repo = NoteRepository(dao)
        val viewModel = NotepadViewModel(repo)

        viewModel.save("C", "D", existingId = 1)
        advanceUntilIdle()

        assertEquals(1, dao.notes.size)
        assertEquals("C", dao.notes[0].title)
    }
}
