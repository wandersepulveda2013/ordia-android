    ...
    companion object {
        ...
        val MIGRATION_2_3 = object : Migration(2, 3) {
            override fun migrate(db: SupportSQLiteDatabase) {
                // SQLite does not support ADD CONSTRAINT for foreign keys, so we must recreate the table.
                db.execSQL("""
                    CREATE TABLE IF NOT EXISTS tasks_new (
                        ...
                        FOREIGN KEY(blockedBy) REFERENCES tasks_new(id) ON UPDATE NO ACTION ON DELETE SET NULL
                    )
                """.trimIndent())
                ...
                db.execSQL("""
                    INSERT INTO tasks_new (
                        ...
                        durationMinutes, priority, status, completed, completedAt,
                        recurrence, recurrenceInterval, recurrenceDays,
                        sortOrder, flagged, archived, createdAt, updatedAt
                    )
                    SELECT id, title, details, projectId, parentTaskId, startAt, dueAt, reminderAt,
                        durationMinutes, priority, status, completed, completedAt,
                        recurrence, recurrenceInterval, recurrenceDays,
                        sortOrder, flagged, archived, createdAt, updatedAt
                    FROM tasks
