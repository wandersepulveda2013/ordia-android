...
private fun wideItems(mode: InterfaceMode): List<Destination> = when (mode) {
    InterfaceMode.SIMPLE -> listOf(
        ...
        Destination.Notes, Destination.Focus, Destination.Search
    )
    InterfaceMode.ORGANIZED -> listOf(
        Destination.Today, Destination.Inbox, Destination.Tasks, Destination.Planner,
        Destination.Projects, Destination.Notes, Destination.Habits, Destination.Focus, Destination.Search
    )
    InterfaceMode.ADVANCED -> listOf(
        Destination.Today, Destination.Inbox, Destination.Tasks, Destination.Planner,
        Destination.Projects, Destination.Notes, Destination.Habits, Destination.Focus,
                ...
                bottomBar = {
                    NavigationBar {
                        compactItems.forEach { item ->
                            val selected = route == item.route || (item == Destination.More && route in setOf(
                                Destination.Projects.route, Destination.Notes.route, Destination.Habits.route,
                                Destination.Focus.route, Destination.Search.route, Destination.Statistics.route, Destination.Archive.route, Destination.Settings.route
                            ))
                            NavigationBarItem(
                                selected = selected,
                                onClick = { navController.navigateSingle(item.route) },
...
private fun OrdiaNavHost(
    ...
    NavHost(
        ...
        composable(Destination.Search.route) { SearchScreen(state, padding, onTask = { navController.navigate(Destination.task(it)) }, onProject = { navController.navigate(Destination.project(it)) }, onNote = { navController.navigate(Destination.note(it)) }, onHabits = { navController.navigateSingle(Destination.Habits.route) }) }
        ...
        composable(Destination.More.route) {
            MoreScreen(
                state = state,
                padding = padding,
                open = { navController.navigateSingle(it) }
            )
        }
        composable(
            ...
            TaskDetailScreen(state, vm, entry.arguments?.getLong("taskId") ?: 0L, padding, onBack = { navController.popBackStack() })
        }
        composable(
            Destination.NOTE_ROUTE,
            arguments = listOf(navArgument("noteId") { type = NavType.LongType })
        ) { entry ->
            NoteEditorScreen(state, vm, entry.arguments?.getLong("noteId") ?: 0L, padding, onBack = { navController.popBackStack() })
        }
        composable(
            Destination.PROJECT_ROUTE,
            arguments = listOf(navArgument("projectId") { type = NavType.LongType })
        ) { entry ->
            ProjectDetailScreen(state, vm, entry.arguments?.getLong("projectId") ?: 0L, padding, onBack = { navController.popBackStack() }, onTask = { navController.navigate(Destination.task(it)) }, onNote = { navController.navigate(Destination.note(it)) })
        }
    }
}
