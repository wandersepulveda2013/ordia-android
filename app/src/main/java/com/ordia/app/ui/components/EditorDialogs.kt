...
import com.ordia.app.ui.components.OrdiaSecondaryButton
import com.ordia.app.ui.components.OrdiaInput
import androidx.compose.material3.Switch
import androidx.compose.material3.Text
import com.ordia.app.ui.components.OrdiaTextButton
    ...
    AlertDialog(
        ...
        text = {
            Column(
                ...
                OrdiaInput(title, { title = it }, modifier = Modifier.fillMaxWidth(), label = "Título", singleLine = true)
                OrdiaInput(details, { details = it }, modifier = Modifier.fillMaxWidth(), label = "Detalles", maxLines = 4)

                Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                    OrdiaSecondaryButton(text = "",
                        onClick = {
                ...
                if (dueAt != null) OrdiaTextButton(text = "Quitar fecha", onClick = { dueAt = null; reminderEnabled = false })

                Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                    Column(Modifier.weight(1f)) {
                        OrdiaSecondaryButton(text = "", onClick = { priorityMenu = true }, modifier = Modifier.fillMaxWidth()) {
                    ...
                    Column(Modifier.weight(1f)) {
                        OrdiaSecondaryButton(text = "", onClick = { projectMenu = true }, modifier = Modifier.fillMaxWidth()) {
                ...
                Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp), verticalAlignment = Alignment.CenterVertically) {
                    Column(Modifier.weight(1f)) {
                        OrdiaSecondaryButton(text = "", onClick = { recurrenceMenu = true }, modifier = Modifier.fillMaxWidth()) {
        ...
        dismissButton = { OrdiaTextButton(text = "Cancelar", onClick = onDismiss) }
    )
}
    ...
    AlertDialog(
        ...
        text = {
            Column(verticalArrangement = Arrangement.spacedBy(10.dp)) {
                OrdiaInput(name, { name = it }, modifier = Modifier.fillMaxWidth(), label = "Nombre", singleLine = true)
                OrdiaInput(description, { description = it }, modifier = Modifier.fillMaxWidth(), label = "Descripción")
        ...
        confirmButton = { OrdiaButton(text = "Guardar", onClick = { onSave((existing ?: ProjectEntity(name = name)).copy(name = name, description = description, status = status)) }, enabled = name.isNotBlank()) },
        dismissButton = { OrdiaTextButton(text = "Cancelar", onClick = onDismiss) }
    )
}
...
fun HabitEditorDialog(existing: HabitEntity? = null, onDismiss: () -> Unit, onSave: (HabitEntity) -> Unit) {
    ...
    AlertDialog(
        ...
        text = {
            Column(verticalArrangement = Arrangement.spacedBy(10.dp)) {
                OrdiaInput(title, { title = it }, modifier = Modifier.fillMaxWidth(), label = "Hábito", singleLine = true)
                OrdiaInput(details, { details = it }, modifier = Modifier.fillMaxWidth(), label = "Por qué es importante")
                Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                    HabitFrequency.entries.forEach { value -> FilterChip(selected = frequency == value, onClick = { frequency = value }, label = { Text(value.label()) }) }
                }
                OrdiaInput(target, { target = it.filter(Char::isDigit).take(2) }, modifier = Modifier.fillMaxWidth(), label = "Meta por período", singleLine = true)
                Row(Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) {
        ...
        confirmButton = { OrdiaButton(text = "Guardar", onClick = { onSave((existing ?: HabitEntity(title = title)).copy(title = title, details = details, frequency = frequency, targetPerPeriod = target.toIntOrNull()?.coerceIn(1, 20) ?: 1, reminderMinutes = if (reminderEnabled) reminderMinutes else null)) }, enabled = title.isNotBlank()) },
        dismissButton = { OrdiaTextButton(text = "Cancelar", onClick = onDismiss) }
    )
}
    ...
    AlertDialog(
        ...
        text = {
            Column(verticalArrangement = Arrangement.spacedBy(10.dp)) {
                OrdiaInput(name, { name = it }, modifier = Modifier.fillMaxWidth(), label = "Nombre", singleLine = true)
                OrdiaInput(description, { description = it }, modifier = Modifier.fillMaxWidth(), label = "Descripción")
                OrdiaInput(steps, { steps = it }, modifier = Modifier.fillMaxWidth(), label = "Pasos, uno por línea")
            }
        },
        confirmButton = { OrdiaButton(text = "Guardar", onClick = { onSave((existing ?: RoutineEntity(name = name)).copy(name = name, description = description), steps.lines()) }, enabled = name.isNotBlank()) },
        dismissButton = { OrdiaTextButton(text = "Cancelar", onClick = onDismiss) }
    )
}
