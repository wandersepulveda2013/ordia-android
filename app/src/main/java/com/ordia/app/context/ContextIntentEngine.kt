...
object ContextIntentEngine {
    ...
    // Piso faena doméstica "quitar las malas hierbas / quitar la hierba"
    // (c.1212 — gap (a) de la auditoría clase VIGESIMOSÉPTIMA, sonda
    // PRE `QuitarHierbasProbe.kt`: NULL medido; segunda sonda del
    // dominio a propósito, hermano de `PodarRosaSetosProbe.kt` c.1211):
    // desherbar es la jardinería tras podar/cortar césped. "quitar"
    // suelto es bivalente (el polvo piso DUST c.732 / la mesa piso
    // CLEAR_TABLE c.754 / el zapato), así se ACOTA al objeto
    // `(?:malas?\s+)?hierbas?` (familia de pisos «quitar» por objeto
    // disjunto — interop: objetos mesa(s)/polvo(s)/hierba(s) sin solape).
    // Lockstep c.751/c.829: keyword-OBJETO «hierbas» añadida en
    // ContextIntent.HOUSEHOLD (sin ella la notificación sin palabra
    // gatillo ni llega al análisis en producción; subcadena cubre el
    // singular, precedente «ruedas»→«rueda» c.1082). El VERBO «quitar»
    // NO se añade (bivalente — precedente c.732/c.754). `\b` final:
    // "hierbajillo"/"hierbabuena" no casan (canaries en el test);
    // guard de negación heredado de la familia (?<!no ). El futuro
    ...
    // tan cotidiana como la vacuna) igual de olvidable. Acotado al
    // objeto mascota `(?:perr[oa]s?|gat[oa]s?)` (familia
    // [HOUSEHOLD_VACCINE_FLOOR] c.757); el destinatario humano queda
    ...
    // Piso jardín "echar abono a las plantas"
    // (c.1220 — lateral ABIERTA de la auditoría clase XXVII c.1211):
    // «echar» es bivalente (a los archivos/el vaso), así se ACOTA a la
    // sustancia (abono|fertilizante) con objeto plantas opcional; sin
    // keyword nueva (gate c.751); MISMA alternancia sustancia/objeto que
    // la plantilla matchEcharAbonoPlantas (lockstep c.616; grafía
    // preservada c.653). `\b` final: objetos fuera de familia no casan
    ...
    // c.1233 (lateral (e) DISJUNTA de c.1232, sonda tools/probe/
    // SalirEnBiciProbe.kt): «salir en (bici|bicicleta)» era NULL (verb-
    // de-posición-libre no-capturado «salir»). Verbo «salir» acotado al
    // objeto-vehículo cerrado (bici|bicicleta), precedente «pilates»
    // c.1232 / «mueble» c.1224. Gate c.751: CERO keywords nuevas.
    // Guard de negación heredado (?<!no ).
    ...
    // c.1116: destino «pediatra» (residual registrado en RUN_LOG c.1106:
    // «llevar al niño al pediatra el viernes» seguía NULL pese a que
    // «pediatra» ya era keyword APPOINTMENT desde c.1106 — el piso cierra
    // el destino con lista propia; el pediatra es el médico de cabecera de
    // los menores, caso de uso central de esta diligencia familiar).
    // Lockstep piso↔plantilla matchMedicalRun (lección c.616); CERO
    // keywords nuevas («niños» preexistente c.773). UNA forma por ciclo:
    // c.1118: destino «dermatólog[oa]» (lateral registrada por ambos lados
    // al cerrar c.1116 — misma causa raíz: el piso cierra el destino con
    // lista propia; «dermatólog[oa]» ya era keyword/listas APPOINTMENT
    // desde c.1110 para la forma propia). Lockstep piso↔plantilla
    // matchMedicalRun (lección c.616); CERO keywords nuevas. UNA forma
    // por ciclo: c.1120: destino «psicólog[oa]» (lateral abierta de
    ...
    private val ERRAND_MEDICAL_RUN_FLOOR =
        ...
        // y el aeropuerto c.1158). c.1178: parentesco adulto directo
        // mamá|papá|madre|padre (el pin c.776 fue SOLO por doctrina
    ...
    // c.1204 — lateral ABIERTA (destino-posesivo) documentada en
    // c.1174/c.1185/c.1192: destino «a mi entrevista». El objeto sigue
    // exigido (guard «llevarme a mi entrevista» → NULL).
    private val ERRAND_INTERVIEW_RUN_FLOOR =
        Regex("""\b(?<!no )(llevarme|llevar|llevo)\s+(?:(?:el|mi|tu|su)\s+)?(?:curr[ií]culum|cv|informe|portfolio)\b\s+a\s+(?:la|mi)\s+entrevista\b""")
    ...
    // c.1256 (lateral (a) FUERTE de MI auditoría c.1252 — clase
    // TRIGÉSIMA SEXTA belleza/cuidado personal): nominal de lugar
    // «(la |mi )?(peluquer[ií]a|barber[ií]a)» era NULL (PRE medido
    // por la sonda persistida `PeluqueriaBarberiaProbe.kt`: 6/6
    // NULL — olvido silencioso P1; el envolvente TASK sólo lo
    // cubre vía «recuérdame»). Gate c.751: objeto
    // monosemántico-lugar (la peluquería/barbería es destino
    // inequívoco de recado); CERO keyword nueva — floor-only
    // (precedente «partido» c.1231 / «clase-fitness» c.1250).
    // Kind hermano de «cortar(me) el pelo» c.842 → ERRAND. «Salón
    ...
    // c.1258 (lateral (c) MEDIA de MI auditoría c.1252 — misma clase
    // XXXVI): el nominal de servicio «depilaci[oó]n» era NULL (PRE
    // medido sonda efímera /tmp/probe1258: 6/6 NULL). Gate c.751:
    // monosemántico-servicio (sesión inequívoca de cuidado); CERO
    // keyword — floor-only; «cera» FUERA (polisémica: vela/coche/oído
    // — paridad «uñas»/«cejas» c.1257, sonda G2; «depilación con cera»
    // queda cubierta por el nominal). La guard
    // [pastErrandCopulaGoverns] cubre el nominal nuevo por la misma
    // constante — lockstep íntegro (piso + plantilla matchBeautyRun).
    private val ERRAND_BEAUTY_RUN_FLOOR =
        Regex("""\b(?<!no )(?:la\s+|mi\s+)?(?:peluquer[ií]a|barber[ií]a|manicura|pedicura|depilaci[oó]n)\b""")
    ...
    // c.1110: «dermatólog[oa]» se añade en lockstep a las cuatro listas médicas
    ...
    // (paridad aceptada del hermano nominal EXERCISE c.1250).
    private val PAST_ERRAND_COPULA_PATTERN = Regex(
        """\b(?:fue|era)\s+(?:ayer|anteayer|anoche)(?!\p{L})"""
    )
    ...
    // c.1072: lateral abierta c.1069/c.1070 — duda intercalada «no sé muy
    // bien si + infinitivo» («no sé muy bien si llamar a mamá» → CALL 0.57
    // firme, «no sé muy bien si ir al médico» → APPOINTMENT 0.67, «no sé muy
    // bien si sacar al perro» → HOUSEHOLD 0.45; 7 capturas medidas PRE con
    // sonda efímera). El «muy bien» entre «sé» y «si» rompía el marcador
    // c.1069. El marcador admite ahora el intercalado opcional «muy bien»;
    // el lookahead de infinitivo (c.1069) y el modal opcional (c.1070) se
    // reutilizan sin tocar. Anti-overreach medido: «sé muy bien que tengo
    // que llamar a mamá» (sin «no») intacto TASK 0.45; «no sé muy bien si
    // es buena idea, llamar a mamá» (la duda gobierna «es buena idea») y
    // «no sé muy bien si debería, llamar a mamá» (la coma cierra) fieles
    // CALL 0.57. Residual aceptado (doctrina de la familia): «no sé muy
    // bien si ir al médico mañana a las 9» 0.85−0.3=0.55 ≥ umbral.
    ...
    // c.1080: lateral abierta c.1078 (1/2) — duda «no sabemos si +
    // MODAL 3ª PERSONA PLURAL + infinitivo» («no sabemos si deberían
    // llamar a mamá» → CALL 0.57 firme, «no sabemos si deberían ir al
    // médico» → APPOINTMENT 0.67 con título corrupto, «no sabemos si
    // podrían sacar al perro» → HOUSEHOLD 0.45, «no sabemos si
    // tendrían que pagar la luz» → TASK 0.45; 7 capturas medidas PRE
    // con sonda efímera, espejo exacto de c.1078 — aquí la acción
    // dudosa es de TERCEROS, no del usuario). El conjunto modal
    // (c.1070 singular, c.1078 1ª plural «…mos») no admitía la 3ª
    // plural «…n». Se admite ahora («deberían (que)», «podrían»,
    // «tendrían que», «habrían que»). Anti-overreach medido:
    // coma-cierra «no sabemos si deberían, llamar a mamá» sigue fiel
    // CALL 0.57; el modal 3ª plural SIN duda intacto; la 2ª persona
    // plural («no sabéis si deberíais…») NO casa (el marcador no
    // admite «sabéis»; lateral hermana 2/2, medida CALL 0.57).
    // Volteo aceptado (consistente con c.1070/c.1078): «no sabemos si
    // deberían haber llamado a mamá» (arrepentimiento pasado) TASK
    // 0.45 → descartado. Residual aceptado (doctrina de la familia):
    // «no sabemos si deberían ir al médico mañana a las 9»
    // 0.85−0.3=0.55 ≥ umbral.
    ...
    private val HEDGE_PATTERN = Regex(
        """(?<!\p{L})(?:quiz[áa]s?|a\s+lo\s+mejor|tal\s+vez|capaz|puede\s+que|a\s+ver\s+si|no\s+(?:s[ée]|sabemos)\s+(?:muy\s+bien\s+)?si(?=\s+(?:(?:(?:deber[ií]a(?:mos|n)?(?:\s+que)?|podr[ií]a(?:mos|n)?|tendr[ií]a(?:mos|n)?\s+que|habr[ií]a(?:mos|n)?\s+que)\s+)?[a-záéíóúñü]*(?:ar|er|ir|ár|ér|ír)(?:me|te|se|le|les|nos|os|lo|la|los|las){0,2}|[a-záéíóúñü]+ré)(?!\p{L})))(?!\p{L})"""
    )
    ...
    private fun scoreKind(lower: String, kind: ContextIntentKind): Float {
        ...
        if (pastErrandCopulaGoverns(lower, kind)) return 0f
    ...
    private fun hasStrongTaskImperative(lower: String): Boolean =
        Regex("""\b(?:recu[ée]rdame|no olvides|tengo (?:que|q)|hay que|(?<!no )cancelar|(?<!no )anular)\s+\w""")
            ...
            // [extractTitle]; las keywords ya existen (correo/email/mensaje).
                        // c.1182: objeto «mail» (candidata (b) auditoría c.1173;
            // anglicismo dominante — «responder el mail» NULL mientras «correo»
...
|| Regex("""(?:^|\b(?:$ACK_PREFIX)\s*[,;.!:]?\s+|\b(?:$TASK_FLOOR_TEMPORAL)\s+)(?<!no )responder\s+(?:al\s+|el\s+|la\s+|los\s+|las\s+|mi\s+|tu\s+|su\s+)?(?:correos?|emails?|mails?|mensajes?|whatsapps?|wasaps?|wassaps?)\b""").containsMatchIn(lower)
            ...
            // c.1177: objeto «whatsapp» (candidata (a) FUERTE auditoría c.1173;
            ...
            // c.1247: piso acotado «verbos brico»
            // (FUERTE de MI auditoría c.1246 clase XXXIV — verbos
            ...
            // PRE medido sobre 5a39f45 con sonda efímera: 4/4 candidatas
            // desnudas NULL, envolvente «recuérdame…» HIT por camino
            // genérico, 6/6 guards NULL, 4/4 regresiones HIT). Hermano
            // EXACTO del piso «sellar (el)? paro» c.1143: kind TASK
            // (gestión laboral SIN desplazamiento explícito; doctrina
            // ERRAND c.842/c.862 solo gobierna el desplazamiento), misma
            // ancla ^|acuse|temporal y guard `(?<!no )`. El objeto
            // EXIGIDO «curr[ií]culums?» (ambas grafías + plural) blinda
            // los bivalentes de «echar» c.829 («echar de menos», «echar
            // la carta al buzón» no casan — NULL deliberado); el pasado
            // «eché…» no casa (regex exige «echar\s»), la negada «no
            // eches…» no casa (morfología distinta), la duda «no sé si
            // echar…» no casa (el ancla exige ^|acuse|temporal antes del
            // verbo). Lockstep keywords-OBJETO «currículum»/«curriculum»
            ...
            // subjuntivo «haga» y 3ª persona «hace» no casan
            ...
            // c.1219: candidata (a) de la clase VIGESIMOQUINTA viajes —
            // «sacar el billete (de tren)» (medida NULL exacta c.1200 y
            // re-medida PRE con sonda persistida
            // `tools/probe/SacarBilleteProbe.kt`: 5/5 candidatas
            // desnudas/temporal/acuse/plural NULL; 2/2 envolventes HIT
            // por c.613; 6/6 guards NULL; 6/6 regresiones HIT). Sin
            // billete no hay viaje: la reserva/compra del desplazamiento
            // es el olvido más caro de la clase junto a salir-aeropuerto
            // c.1150 y facturar-vuelo c.1140. Objeto EXIGIDO «(el|los)
            // billete(s)» (anti-overreach: el bivalente «sacar el
            // pasaporte» queda FUERA — lateral; «sacar» es el verbo con
            // más pisos acotados del motor, todos con objeto-ancla).
            // Hermano EXACTO de «sacar el visado» c.1151. Lockstep
            // keywords-frase en ContextIntent.TASK + plantilla en
            // extractTitle (lección c.616). La negada la cubre el
            // lookbehind `(?<!no )` (keyword 0.12 + bono 0.1 = 0.22 <
            // umbral: sin cláusula dedicada en [imperativeIsNegated],
            // mismo argumento que c.895b/c.895c/c.1140); el pasado
            // «saqué» y el sustantivo «el billete de tren» no casan
            // (verbo EXIGIDO en infinitivo). Kind TASK (reserva previa
     ...
     * manicura|pedicura|depilaci[oó]n)» (c.1256 + extensiones c.1257/c.1258
     * por la MISMA constante) está GOBERNADO por una copulativa en pretérito pospuesta
     ...
     * candado c.613) sobrevive al descarte. Acotado al piso nominal para
     * no tocar los pisos ERRAND verbales ni el comportamiento de otros
     * kinds. «estuvo llena/cerrada ayer» y «fui a (la) peluquería» quedan
     * laterales ABIERTAS (paridad aceptada del hermano nominal EXERCISE
     * c.1250) — una forma por ciclo.
     */
    ...
    private fun extractTitle(original: String, kind: ContextIntentKind): String? {
        ...
        return when (kind) {
            ContextIntentKind.TASK -> {
                ...
                // ancla/guard, doctrina c.653: verbo-frase preservado,
                // solo capitalización inicial; el residuo temporal de
                // cola lo depura [sanitizeTitle]).
                ...
                // c.1219: plantilla hermana del piso «sacar (el|los)
                // billete(s)» (ancla/guard idénticos; lección c.616: el
                // match arranca en el verbo, así acuse/prefijo temporal
                // se despojan). Captura SOLO el objeto + calificador
                // opcional «de|del <producto-monosemántico>» («de tren»,
                // «del bus» — grafía del usuario, doctrina c.653): la
            ...
            ContextIntentKind.EXERCISE -> {
                ...
                // c.1233: «salir en (bici|bicicleta)» (lateral (e)
                // DISJUNTA de c.1232) — piso acotado al verbo «salir»
                // en [EXERCISE_BIKE_OUT_FLOOR]. El match arranca en el
                // verbo, así el prefijo «voy a» no ensucia el título.
                val matchSalirEnBici = Regex(
                    """\b(?<!no )(salir\s+en\s+(?:bici|bicicleta).*)""",
                    RegexOption.IGNORE_CASE
                ).find(original)
            ...
            ContextIntentKind.HOUSEHOLD -> {
                ...
                // "quitar (las) (malas) hierba(s) …" → "Quitar las malas
                // hierbas …" (c.1212): verbo preservado y objeto restringido
                // como en [HOUSEHOLD_WEED_FLOOR] (tercer piso de la familia
                // «quitar» por objeto disjunto: polvo c.732 / mesa c.754 /
                // hierbas aquí — no colisionan). Alternancia de artículo
                // ARTÍCULO DIRECTO como en el piso (mi|tu|su, c.756).
                val matchQuitarHierbas = Regex(
                    """\b(?<!no )(quitar)\s+((?:el\s+|la\s+|los\s+|las\s+|mi\s+|tu\s+|su\s+)?(?:malas?\s+)?hierbas?\b.*)""",
                    RegexOption.IGNORE_CASE
                ).find(original)
                ...
                if (matchCastrarMascota != null) {
                    return "${capitalizeFirst(matchCastrarMascota.groupValues[1])} ${matchCastrarMascota.groupValues[2]}"
                }
                ...
                // Piso "echar abono a las plantas" (c.1220 — lateral ABIERTA
                // auditoría clase XXVII c.1211): MISMA alternancia sustancia/
                // (objeto opcional) que el piso [HOUSEHOLD_FERTILIZE_FLOOR]
                // (lockstep piso↔plantilla lección c.616; grafía
                // preservada c.653).
                val matchEcharAbonoPlantas = Regex(
                    """\b(echar) ((?:(?:el|la|los|las|un|una|unos|unas) )?(?:abono|fertilizante)(?:\s+a\s+(?:las|los|tus|mis|sus|el|un|una)\s+(?:planta(?:s)?|suculenta(?:s)?|rosal(?:es)?|flor(?:es)?|jard(?:ín|ines)))?.*)""",
                    RegexOption.IGNORE_CASE
                ).find(original)
            ...
            ContextIntentKind.ERRAND -> {
                ...
                // c.1204: destino-posesivo «a (la|mi) entrevista»
                // (lockstep con [ERRAND_INTERVIEW_RUN_FLOOR]).
                val matchInterviewRun = Regex(
                    """(?:^|\b(?:$ACK_PREFIX)\s*[,;.!:]?\s+|\b(?:$TASK_FLOOR_TEMPORAL)\s+)(?<!no )(llevarme|llevar|llevo)\s+((?:(?:(?:el|mi|tu|su)\s+)?(?:curr[ií]culum|cv|informe|portfolio)\b\s+a\s+(?:la|mi)\s+entrevista).*)""",
                    RegexOption.IGNORE_CASE
                ).find(original)
                ...
                // "peluquería / la peluquería / mi barbería / manicura /
                // pedicura / depilación" → "Peluquería" / "La peluquería" /
                // "Mi barbería" / "La depilación" (c.1256 + c.1257 + c.1258,
                // lockstep DOS puntos con [ERRAND_BEAUTY_RUN_FLOOR]): el
                // match arranca en el nominal de lugar o servicio, así el
                // acuse/prefijo temporal ("en la peluquería el viernes",
                // "cita para la depilación el jueves") no ensucia el título
                // (lección c.616 — paridad matchClase c.1250); residuo
                // temporal de cola depurado por [sanitizeTitle].
                val matchBeautyRun = Regex(
                    """\b(?<!no )((?:la\s+|mi\s+)?(?:peluquer[ií]a|barber[ií]a|manicura|pedicura|depilaci[oó]n)\b.*)""",
                    RegexOption.IGNORE_CASE
                ).find(original)
