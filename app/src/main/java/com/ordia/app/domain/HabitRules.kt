...
object HabitRules {
    fun isScheduled(habit: HabitEntity, date: LocalDate): Boolean {
        ...
        return when (habit.frequency) {
            HabitFrequency.DAILY -> days.isEmpty() || date.dayOfWeek.value in days
            HabitFrequency.WEEKLY -> days.isEmpty() || date.dayOfWeek.value in days
    ...
    fun currentStreak(habit: HabitEntity, logs: List<HabitLogEntity>, today: LocalDate = LocalDate.now()): Int {
        ...
        while (guard++ < 730) {
            ...
            if (date.toEpochDay() in completed) {
                streak++
                date = date.minusDays(1)
            } else break
        }
